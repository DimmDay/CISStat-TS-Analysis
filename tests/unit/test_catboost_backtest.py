"""Task 130 -- CatBoost в движке бэктеста и тюнинга (real registry).

E2E-контракт вертикального среза (зеркало Task 127/128/129):
- catboost зарегистрирован в MODEL_EXECUTION_REGISTRY как supervised-адаптер
...
def _backtest_plan() -> tuple[object, list[float], list[str]]:
    ...
    backtest_plan = build_backtest_plan(
        ...
        fingerprint="task130-cb", target_column="value", seasonal_period=12,
        feature_plan=plan, feature_columns=_feature_columns(labels),
    )
    return backtest_plan, values, labels


CB_PARAMS = {"iterations": 40, "n_lags": 3}
...
class TestCatboostRegistry:
    def test_registered_as_supervised_ml_adapter(self):
        descriptor = MODEL_EXECUTION_REGISTRY.describe("catboost")
        assert descriptor["family_id"] == "tree_ml"
        assert descriptor["adapter_id"] == "catboost-native"
        ...
        assert descriptor["engine"] == "catboost"
        assert descriptor["required_packages"] == ["catboost"]
        assert descriptor["runtime_available"] is True
        assert set(descriptor["actions"]) == {"backtest", "tune", "diagnostics"}

    def test_production_actions_include_catboost(self):
        assert "catboost" in PRODUCTION_BACKTEST_MODEL_IDS
        assert "catboost" in PRODUCTION_TUNING_MODEL_IDS
...
class TestCatboostBacktest:
    ...
    def test_folds_record_importance_bound_to_exact_matrix(self):
        ...
        for fold in result["folds"]:
            ...
            assert "cb_lag_1" in names
            assert "date_month_sin" in names
            total = sum(item["importance"] for item in importance["importances"])
            # Контракт нормализации: адаптер приводит сырые проценты
            # PredictionValuesChange (сумма 100) CatBoost к сумме 1.0.
    ...
    def test_run_without_feature_plan_still_works(self):
        ...
        plan = build_backtest_plan(
            _validation(len(values)), n_observations=len(values),
            fingerprint="task130-cb-bare", target_column="value", seasonal_period=12,
        )
        result = run_backtest_plan(
            model_id="catboost", model_name="CatBoost",
            family_id="tree_ml", series=values, labels=labels, plan=plan,
...
class TestCatboostTuning:
    ...
    def test_tuning_runs_on_same_folds_and_picks_best(self):
        ...
        response = execute_tuning_plan(
            model_id="catboost", model_name="CatBoost",
            family_id="tree_ml",
            param_space={"iterations": [30, 50], "n_lags": [2, 3]},
        ...
        assert response.best_params["iterations"] in {30, 50}
