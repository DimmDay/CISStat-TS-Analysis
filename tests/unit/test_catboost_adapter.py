"""Task 130 -- unit-тесты CatBoost адаптера (recursive supervised ML).
...
- bounded params CatBoost (iterations/depth/learning_rate/l2_leaf_reg/
  bagging_temperature/random_strength/n_lags) -- fail-closed;
- детерминизм (fixed seed) и отсутствие скрытой стохастичности при
  random_strength=0 (bagging_temperature=0.0 -- дефолт адаптера): разные
  seed -- одинаковый прогноз; при дефолтном random_strength=1.0 (официальный
  дефолт CatBoost) seed реально влияет на прогноз (проводка random_state
  до бустера);
- prediction intervals через quantile regression (loss_function=
  "Quantile:alpha=0.1/0.9", fixed 80%): lower <= point <= upper, честная
  ширина на шумных данных;
- период-2 закрытая форма [1,2,1] с машинной точностью (детерминированно
  ловит stale-history/off-by-one рекурсии);
- feature importance (PredictionValuesChange) НОРМАЛИЗОВАН адаптером:
  нативный get_feature_importance() возвращает проценты с суммой 100 --
  контракт платформы требует сумму 1.0; привязка к ТОЧНОЙ fold-матрице
  (bind_feature_importance, oracle-отрицательный контроль);
- деградация fail-closed: деградированные данные CatBoost ОТКЛОНЯЕТ на
  уровне библиотеки (CatBoostError) -- ошибка поднимается как есть,
  никаких синтетических fallback-метрик (рекомендация сертификации
  Task 129: dedicated-тест деградированной ветки);
...
from apps.api.model_impls.catboost import (
    DEFAULT_N_LAGS,
    PARAM_BOUNDS,
    _cb_fit_predict,
    cb_feature_specs,
...
def _y(n: int = 48) -> list[float]:
    """Тренд + сезон + детерминированный джиттер: шум обязателен, чтобы
    квантильные модели давали нетривиальную ширину интервалов."""
    return [
...
class TestCbFeatureSpecs:
    def test_specs_cover_lags_rolling_and_difference_with_prefix(self):
        specs = cb_feature_specs(n_lags=3)
        assert set(specs) == {
            "cb_lag_1", "cb_lag_2", "cb_lag_3",
            "cb_roll_mean", "cb_roll_std", "cb_diff_1",
        }
        assert specs["cb_lag_2"].kind == KIND_LAG
        assert specs["cb_lag_2"].role == ROLE_HISTORIC
        assert int(specs["cb_lag_2"].lookback) == 2
        assert specs["cb_roll_mean"].kind == KIND_ROLLING
        assert int(specs["cb_roll_mean"].lookback) == 3
        assert str(specs["cb_roll_mean"].params["statistic"]) == "mean"
        assert specs["cb_diff_1"].kind == KIND_DIFFERENCE
        assert int(specs["cb_diff_1"].lookback) == 2
    ...
    def test_shared_core_matrix_is_causal_with_warmup(self):
        ...
        columns, rows, target, warmup = supervised_matrix(
            y, known={}, specs=cb_feature_specs(n_lags=3),
        )
        assert warmup == 3
    ...
    def test_known_columns_are_aligned_and_sorted(self):
        ...
        columns, rows, _target, warmup = supervised_matrix(
            y, known=known, specs=cb_feature_specs(n_lags=2),
        )
        assert columns[len(cb_feature_specs(2)):] == ["alpha", "zeta"]
    ...
    def test_insufficient_history_fails_closed(self):
        with pytest.raises(ValueError, match="истор"):
            supervised_matrix(
                [1.0, 2.0, 3.0], known={}, specs=cb_feature_specs(n_lags=2),
            )
...
class TestValidateCbParams:
    def test_defaults_are_valid_and_normalized(self):
        params = validate_cb_params({})
        assert params["iterations"] == 200
        assert params["depth"] == 6
        assert params["learning_rate"] == pytest.approx(0.1)
        assert params["l2_leaf_reg"] == pytest.approx(3.0)
        assert params["bagging_temperature"] == pytest.approx(0.0)
        assert params["random_strength"] == pytest.approx(1.0)
        assert params["n_lags"] == DEFAULT_N_LAGS

    def test_valid_params_pass_through(self):
        params = validate_cb_params({
            "iterations": 50, "depth": 4, "learning_rate": 0.3,
            "l2_leaf_reg": 0.5, "bagging_temperature": 2.0,
            "random_strength": 0.0, "n_lags": 3,
        })
        assert params["iterations"] == 50
        assert params["depth"] == 4
        assert params["learning_rate"] == pytest.approx(0.3)
        assert params["l2_leaf_reg"] == pytest.approx(0.5)
        assert params["bagging_temperature"] == pytest.approx(2.0)
        assert params["random_strength"] == pytest.approx(0.0)
        assert params["n_lags"] == 3

    @pytest.mark.parametrize("override", [
        {"iterations": 5}, {"iterations": 2001},
        {"depth": 0}, {"depth": 17},
        {"learning_rate": 0.0005}, {"learning_rate": 1.5},
        {"l2_leaf_reg": -0.1}, {"l2_leaf_reg": 1001.0},
        {"bagging_temperature": -0.1}, {"bagging_temperature": 101.0},
        {"random_strength": -0.1}, {"random_strength": 101.0},
        {"n_lags": 0}, {"n_lags": 33},
        {"iterations": 50.5}, {"depth": 3.5}, {"n_lags": "many"},
        {"learning_rate": "fast"},
        {"depth": True}, {"bagging_temperature": False},
    ])
    def test_out_of_bounds_params_fail_closed(self, override):
        with pytest.raises(ValueError):
            validate_cb_params(override)
...
# ══════════════════════════════════════════════════════════════════════════════
# _cb_fit_predict: рекурсия, детерминизм, интервалы, importance
# ══════════════════════════════════════════════════════════════════════════════

class TestCbFitPredictHappyPath:
    def test_forecast_is_deterministic_for_fixed_seed(self):
        y = _y(48)
        first = _cb_fit_predict(y, 6, params={"iterations": 40, "n_lags": 3}, random_state=42)
        second = _cb_fit_predict(y, 6, params={"iterations": 40, "n_lags": 3}, random_state=42)
    ...
    def test_zero_score_noise_has_no_hidden_stochasticity(self):
        """random_strength=0 (score-шум выключен), bagging_temperature=0.0
        (дефолт адаптера, байесовский бутстрап строк выключен): бустер
        детерминирован независимо от seed.  Честное доказательство
        отсутствия скрытой случайности; при дефолтном random_strength=1.0
        seed реально доходит до бустера (следующий тест).
        """
        y = _y(48)
        by_seed = [
            _cb_fit_predict(
                y, 4,
                params={"iterations": 30, "n_lags": 2, "random_strength": 0.0},
                random_state=seed,
            )
    ...
    def test_seed_reaches_the_booster_with_default_score_noise(self):
        """Дефолт адаптера random_strength=1.0 (официальный дефолт CatBoost):
        разные seed дают разные прогнозы -- random_state доходит до бустера.
        """
        y = _y(48)
        first = _cb_fit_predict(
            y, 4, params={"iterations": 40, "depth": 4, "n_lags": 2}, random_state=42,
        )
        second = _cb_fit_predict(
            y, 4, params={"iterations": 40, "depth": 4, "n_lags": 2}, random_state=7,
        )
        assert first["forecast"] != second["forecast"]

    def test_forecast_shape_and_intervals_contain_point(self):
        y = _y(48)
        payload = _cb_fit_predict(y, 5, params={"iterations": 40, "n_lags": 3}, random_state=42)
    ...
    def test_recursive_period_two_continuation_is_exact(self):
        ...
        push).  learning_rate=1.0 + l2_leaf_reg=0 + random_strength=0:
        бустинг с boost_from_average достигает нулевых остатков на чистых
        листьях (симметричные oblivious-деревья CatBoost).
        """
        y = [1.0, 2.0] * 6
        payload = _cb_fit_predict(
            y, 3,
            params={
                "iterations": 30, "depth": 3, "learning_rate": 1.0,
                "l2_leaf_reg": 0.0, "bagging_temperature": 0.0,
                "random_strength": 0.0, "n_lags": 2,
            },
            random_state=42,
        )
    ...
    def test_forecast_stays_within_training_target_range(self):
        ...
        payload = _cb_fit_predict(
            y, 4,
            params={"iterations": 30, "depth": 6, "learning_rate": 0.5, "n_lags": 2},
            random_state=42,
        )
        assert min(payload["forecast"]) >= min(y) - 1e-9
        assert max(payload["forecast"]) <= max(y) + 1e-9

    def test_constant_target_fails_closed_at_library_level(self):
        """Деградированная ветка (рекомендация сертификации Task 129):
        CatBoost ОТКАЗЫВАЕТСЯ обучаться на деградированных данных и ошибка
        поднимается вызывающей стороне как есть -- синтетические
        fallback-метрики запрещены (тот же стандарт, что у
        XGBoost/LightGBM).

        Через этот адаптер константный target обязан давать константные
        target-derived признаки (лаги/rolling/diff -- функции target),
        поэтому CatBoost падает на стадии квантизации
        ("All features are either constant or ignored"); чисто target-ная
        деградация ("All train targets are equal") достижима только при
        варьирующихся признаках и константном target -- через
        target-derived конструирование признаков недостижима.  Обе ветки
        документированы: library-native fail-closed, матч покрывает обе."""
        y = [7.0] * 24
        with pytest.raises(Exception, match="All (features|train targets)"):
            _cb_fit_predict(y, 2, params={"iterations": 30, "n_lags": 2}, random_state=42)
    ...
    def test_feature_importance_is_normalized_and_bound_to_exact_matrix(self):
        ...
        payload = _cb_fit_predict(
            ...
            params={"iterations": 40, "n_lags": 3}, random_state=42,
        )
        lineage = payload["feature_importance_lineage"]
        records = payload["feature_importances"]
        assert lineage["adapter_id"] == "catboost-native"
        ...
        # Нативный get_feature_importance() CatBoost возвращает
        # PredictionValuesChange в процентах с суммой 100 -- адаптер обязан
        # нормализовать к сумме 1.0 (контракт платформы).
...
# ══════════════════════════════════════════════════════════════════════════════
# _cb_fit_predict: fail-closed контракт
# ══════════════════════════════════════════════════════════════════════════════

class TestCbFitPredictFailClosed:
    def test_nan_target_rejected(self):
        with pytest.raises(ValueError, match="NaN/Inf"):
            _cb_fit_predict([1.0, math.nan, 3.0, 4.0, 5.0, 6.0, 7.0, 8.0,
    ...
    def test_future_without_train_regressors_rejected(self):
        ...
        with pytest.raises(ValueError, match="симметрично"):
            _cb_fit_predict(y, 2, future_features={"x": [1.0, 2.0]},
                            params={"n_lags": 2}, random_state=42)

    def test_train_without_future_regressors_rejected(self):
        y = _y(24)
        with pytest.raises(ValueError, match="horizon"):
            _cb_fit_predict(y, 2, train_features={"x": [1.0] * len(y)},
                            params={"n_lags": 2}, random_state=42)
    ...
    def test_insufficient_train_history_rejected(self):
        ...
        with pytest.raises(ValueError, match="истор"):
            _cb_fit_predict(y, 2, params={"n_lags": 4}, random_state=42)
