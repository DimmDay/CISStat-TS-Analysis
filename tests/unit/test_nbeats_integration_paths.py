...
"""Task 139 -- N-BEATS: интеграция реестра v2, dispatch, readiness,
матрицы применимости, yaml-спецификации и session-движка (второй
исполнитель neural-runtime контракта Task 137).

Прецедент пары lstm (Task 138): runtime-контракт Task 137
...
- dispatch: _BACKTEST_IMPLEMENTATIONS согласован с реестром (gate) в
  ОБЕИХ средах -- neural-записи (lstm, nbeats) регистрируются УСЛОВНО
  пробу neuralforecast_runtime_available() (соглашение регистрации
  _register_neural_dispatch прижимается тестами с обеих сторон);
- readiness: nbeats в PRODUCTION_BACKTEST_MODEL_IDS <=> runtime
  установлен (21 connected на neural-воркере; на хосте без группы
  модель честно остаётся catalog_only);
- yaml: bounded param_space stack_config x hidden_size x input_size =
  8 trials (<= MAX_TRIALS=64), значения -- внутри адаптерных границ;
...
from apps.api.model_impls import run_nbeats_backtest as exported_run_nbeats_backtest
...
def test_nbeats_registry_entry_contract():
    descriptor = MODEL_EXECUTION_REGISTRY.describe("nbeats")
    assert descriptor["model_id"] == "nbeats"
...
def test_nbeats_runtime_availability_is_the_honest_package_probe():
    definition = MODEL_EXECUTION_REGISTRY.require("nbeats")
    assert definition.runtime_available() is _HAS_NEURAL
    assert ("nbeats" in MODEL_EXECUTION_REGISTRY.model_ids_for("backtest")) is _HAS_NEURAL


def test_nbeats_registry_rejects_wrong_objective_and_features_fail_closed():
    ...
    with pytest.raises(ModelExecutionContractError, match="objective"):
        MODEL_EXECUTION_REGISTRY.execute("nbeats", request)
    ...
    with pytest.raises(ModelExecutionContractError, match="train_features"):
        MODEL_EXECUTION_REGISTRY.execute("nbeats", feature_request)


# ── Readiness и матрица применимости ─────────────────────────────────────

def test_nbeats_readiness_and_stage_matrix_reflect_runtime():
    actions = available_model_actions("nbeats")
    capabilities = model_stage_capabilities("nbeats", "neural")
    if _HAS_NEURAL:
...
@requires_neural
def test_nbeats_executor_returns_contract_result_with_metadata(monkeypatch):
    import apps.api.model_impls.nbeats as nbeats_module

    monkeypatch.setattr(nbeats_module, "NBEATS_MAX_STEPS", 3)
    result = MODEL_EXECUTION_REGISTRY.execute("nbeats", ModelExecutionRequest(
    ...
    assert result.metadata["adapter_id"] == "neuralforecast-nbeats"
    assert result.metadata["stack_config"] == "interpretable"
    assert result.metadata["deterministic"] is True


@requires_neural
def test_nbeats_executor_runs_through_the_session_engine(monkeypatch):
    import math
    ...
    plan = build_backtest_plan(
        validation, n_observations=len(series), fingerprint="nbeats-series",
...
def test_legacy_dispatch_export_executes_real_metrics(monkeypatch):
    ...
    metrics = exported_run_nbeats_backtest(series, 0.75, 12)
    assert metrics.mae is not None


def test_grid_product_of_yaml_space_is_eight_trials():
    """8 = 2 x 2 x 2 -- фиксированный bounded grid Task 139
    (честная альтернатива стеков прижимается декларацией)."""
    from src.catalog.modeling_spec_loader import ModelingSpec
