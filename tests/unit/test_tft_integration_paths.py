...
"""Task 141 -- TFT: интеграция реестра v2, dispatch, readiness,
матрицы применимости, yaml-спецификации и session-движка (четвёртый
исполнитель neural-runtime контракта Task 137, ПЕРВЫЙ срез с
probabilistic-поверхностью MQLoss/quantiles).

Прецедент тройки lstm/nbeats/nhits (Tasks 138/139/140): runtime-контракт
Task 137 (neural_runtime.py + neural_contract.py) НЕ меняется -- новый
адаптер + запись реестра + условный dispatch + yaml:
...
- dispatch: _BACKTEST_IMPLEMENTATIONS согласован с реестром (gate) в
  ОБЕИХ средах -- neural-записи (lstm, nbeats, nhits, tft) регистрируются
...
Дополнительно (постановка Task 141: probabilistic-поверхность
MQLoss/quantiles): quartet исполнителей живёт в ОДНОМ level-cohort
(objective/input_kind/engine/dependency_group идентичны) -- метрики
точечных прогнозов ранжируемы напрямую; ПРОИСХОЖДЕНИЕ интервалов
честно различается в metadata: tft -- native quantiles
(NeuralIntervalPlan.method="neural_quantile_outputs"), тройка
lstm/nbeats/nhits -- conformal.  Различие задекларировано, а не скрыто.
"""
...
from apps.api.model_impls import run_tft_backtest as exported_run_tft_backtest
...
def test_neural_dispatch_registration_convention():
    ...
    реестр<->dispatch остаётся точным), с runtime -- появляются все
    четыре (Task 138 lstm + Task 139 nbeats + Task 140 nhits + Task 141
    tft)."""
    without: dict = {}
...
def test_dispatch_gate_consistency_in_this_environment():
    ...
    assert ("tft" in _BACKTEST_IMPLEMENTATIONS) is _HAS_NEURAL


# ── Реестр v2: контракт neural ───────────────────────────────────────────

def test_tft_registry_entry_contract():
    descriptor = MODEL_EXECUTION_REGISTRY.describe("tft")
    assert descriptor["model_id"] == "tft"
...
def test_tft_shares_one_level_cohort_with_the_neural_family():
    """Постановка Task 141: quartet исполнителей имеет ИДЕНТИЧНЫЙ
    когортный контракт (objective/input_kind/engine/dependency_group,
    actions, пакеты, детерминизм) и ЕДИНУЮ точку исполнения --
    train_and_forecast neural_runtime (адаптеры не несут собственной
    fit/predict-петли): все четыре модели попадают в один level-cohort
    OOF-бэктеста на одних и тех же fold'ах -- comparison sectioned by
    objective ранжирует quartet честно, без выравнивания контрактов."""
    ...
    for key in ("engine", "objective", "input_kind", "dependency_group",
        ...
        assert tft[key] == nhits[key] == nbeats[key], key
    ...
    # ЕДИНСТВЕННАЯ точка исполнения: все адаптеры ссылаются на один
    # и тот же runtime-символ (не собственные копии fit/predict-петли).
    assert tft_module.train_and_forecast is nhits_module.train_and_forecast
    assert tft_module.train_and_forecast is nbeats_module.train_and_forecast
    assert tft_module.train_and_forecast.__module__ == neural_runtime.__name__
    # ds-конвенция -- единый источник истины (не дубликат).
    assert tft_module._resolve_time_axis is nhits_module._resolve_time_axis
    # Адаптер не импортирует torch на уровне модуля.
    assert "torch" not in vars(tft_module)


def test_tft_intervals_are_native_quantiles_not_conformal():
    """Честное различие ПРОИСХОЖДЕНИЯ интервалов внутри одного cohort'а:
    tft -- нативные квантили MQLoss (метод NeuralIntervalPlan контракта
    "neural_quantile_outputs"); тройка lstm/nbeats/nhits -- сертифицированный
    conformal-контур ("conformal").  Различие задекларировано в payload
    адаптера, а не скрыто: ranking по точечным прогнозам не нарушен."""
    import apps.api.model_impls.nhits as nhits_module
    import apps.api.model_impls.tft as tft_module

    tft_intervals = tft_module._quantile_plan(0.05)
    assert tft_intervals["method"] == "neural_quantile_outputs"
    assert tft_intervals["loss"] == "mqloss"
    # Сравнение с conformal-прецедентом: ключи метода различны, альфа-план
    # один и тот же (interval_levels_for_alpha 0.05 -> (2.5, 50.0, 97.5)).
    nhits_intervals_method = "conformal"
    assert tft_intervals["method"] != nhits_intervals_method
    assert list(tft_intervals["levels"]) == [2.5, 50.0, 97.5]


def test_tft_runtime_availability_is_the_honest_package_probe():
    definition = MODEL_EXECUTION_REGISTRY.require("tft")
    assert definition.runtime_available() is _HAS_NEURAL
    assert ("tft" in MODEL_EXECUTION_REGISTRY.model_ids_for("backtest")) is _HAS_NEURAL


def test_tft_registry_rejects_wrong_objective_and_features_fail_closed():
    """Гейты реестра v2 работают ДО запуска executor'а (не требуют
    neural-runtime): objective=multivariate -- отказ; train_features для
    univariate-входа -- отказ (exog-канал нейро-моделей -- отдельная
    постановка, прецедент lstm/nbeats/nhits)."""
    request = ModelExecutionRequest(
        target=[1.0, 2.0, 3.0], horizon=2, objective="multivariate",
    )
    with pytest.raises(ModelExecutionContractError, match="objective"):
        MODEL_EXECUTION_REGISTRY.execute("tft", request)
    ...
    with pytest.raises(ModelExecutionContractError, match="train_features"):
        MODEL_EXECUTION_REGISTRY.execute("tft", feature_request)


# ── Readiness и матрица применимости ─────────────────────────────────────

def test_tft_readiness_and_stage_matrix_reflect_runtime():
    actions = available_model_actions("tft")
    capabilities = model_stage_capabilities("tft", "neural")
    if _HAS_NEURAL:
...
def test_yaml_param_space_is_bounded_and_within_adapter_bounds():
    ...
    assert set(space["n_head"]) <= set(N_HEAD_OPTIONS)
    ...
    # Constraint делимости d_k выполним на ВСЕМ декларированном grid'е.
    for head in space["n_head"]:
        assert all(value % head == 0 for value in space["hidden_size"])
...
def test_yaml_catalog_entry_keeps_the_honest_name():
    ...
    spec = ModelingSpec.from_yaml("rules/modeling.yaml")
    model = spec.get_model("tft")
    assert model.name == "Temporal Fusion Transformer"
    assert model.supports_prediction_intervals is True


def test_grid_product_of_yaml_space_is_eight_trials():
    """8 = 2 x 2 x 2 -- фиксированный bounded grid Task 141
    (attention-ось n_head x capacity x окно прижимается декларацией)."""
    from src.catalog.modeling_spec_loader import ModelingSpec
...
@requires_neural
def test_tft_executor_returns_contract_result_with_metadata(monkeypatch):
    import apps.api.model_impls.tft as tft_module

    monkeypatch.setattr(tft_module, "TFT_MAX_STEPS", 20)
    result = MODEL_EXECUTION_REGISTRY.execute("tft", ModelExecutionRequest(
...
@requires_neural
def test_tft_executor_runs_through_the_session_engine(monkeypatch):
    import math
    ...
    plan = build_backtest_plan(
        validation, n_observations=len(series), fingerprint="tft-series",
    ...
    result = run_backtest_plan(
        ...
        family_id="neural", series=series,
...
@requires_neural
def test_neural_family_runs_the_same_oof_cohort_for_fair_comparison(monkeypatch):
    """E2E-оракул базы сравнения quartet'а: tft на ОДНОМ runtime проходит
    ОДИН и тот же план fold'ов (build_backtest_plan fingerprint-независим
    от модели) в одном level-cohort с nbeats/nhits -- метрики ранжируемы
    напрямую; различие интервалов -- только в provenance metadata."""
    import math
    ...
    plan = build_backtest_plan(
        validation, n_observations=len(series), fingerprint="quartet-compare",
        target_column="value", seasonal_period=12,
    )
    names = {"nbeats": "N-BEATS", "nhits": "N-HiTS",
             "tft": "Temporal Fusion Transformer"}
    results = {
        model_id: run_backtest_plan(
            model_id=model_id, model_name=names[model_id],
    ...
    assert contracts["tft"]["adapter_id"] == "neuralforecast-tft"
...
def test_legacy_dispatch_export_executes_real_metrics(monkeypatch):
    ...
    metrics = exported_run_tft_backtest(series, 0.75, 12)
