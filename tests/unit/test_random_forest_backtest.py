"""Task 127 -- Random Forest в движке бэктеста и тюнинга (real registry).

E2E-контракт вертикального среза:
- random_forest зарегистрирован в MODEL_EXECUTION_REGISTRY как supervised-
  адаптер с supports_future_features: fold-local FeaturePlan передаёт ему
...
def _backtest_plan() -> tuple[object, list[float], list[str]]:
    ...
    backtest_plan = build_backtest_plan(
        ...
        fingerprint="task127-rf", target_column="value", seasonal_period=12,
        feature_plan=plan, feature_columns=_feature_columns(labels),
    )
    return backtest_plan, values, labels


RF_PARAMS = {"n_estimators": 60, "n_lags": 3}
...
class TestRandomForestRegistry:
    def test_registered_as_supervised_ml_adapter(self):
        descriptor = MODEL_EXECUTION_REGISTRY.describe("random_forest")
        assert descriptor["family_id"] == "tree_ml"
        assert descriptor["adapter_id"] == "sklearn-random-forest"
        ...
        assert descriptor["engine"] == "scikit-learn"
        assert "scikit-learn" in descriptor["required_packages"]
        assert descriptor["runtime_available"] is True
        assert set(descriptor["actions"]) == {"backtest", "tune", "diagnostics"}

    def test_production_actions_include_random_forest(self):
        assert "random_forest" in PRODUCTION_BACKTEST_MODEL_IDS
        assert "random_forest" in PRODUCTION_TUNING_MODEL_IDS

    def test_univariate_models_reject_future_features(self):
        """Отрицательный контроль: гейт Task 126 не ослаблен."""
        ...
        with pytest.raises(ModelExecutionContractError, match="train_features"):
            MODEL_EXECUTION_REGISTRY.execute("naive", ModelExecutionRequest(
                target=[1.0, 2.0, 3.0], horizon=1,
                train_features={"x": [1.0, 2.0, 3.0]}, future_features={"x": [4.0]},
            ))
...
class TestRandomForestBacktest:
    def test_run_backtest_plan_end_to_end(self):
        plan, values, labels = _backtest_plan()
        result = run_backtest_plan(
            model_id="random_forest", model_name="Random Forest",
            family_id="tree_ml", series=values, labels=labels, plan=plan,
            seasonal_period=12, params=RF_PARAMS,
        )
        assert result["status"] == "success"
        assert result["n_folds"] == 2
        # Regressor-канал granted: ни одного exclusion-warning от FeaturePlan.
        feature_warnings = [
    ...
    def test_folds_record_importance_bound_to_exact_matrix(self):
        ...
        for fold in result["folds"]:
            ...
            # Plan lineage (Task 126) по-прежнему в fold-записи.
            ...
            # Importance покрывает и lag-признаки адаптера, и future_known-колонку плана.
            assert "rf_lag_1" in names
    ...
    def test_metrics_are_computed_not_fabricated(self):
        ...
        assert metrics["weighted_score"] is None  # взвешенный score -- только comparison

    def test_run_without_feature_plan_still_works(self):
        values, labels = _series()
        plan = build_backtest_plan(
            _validation(len(values)), n_observations=len(values),
            fingerprint="task127-rf-bare", target_column="value", seasonal_period=12,
        )
        result = run_backtest_plan(
            model_id="random_forest", model_name="Random Forest",
            family_id="tree_ml", series=values, labels=labels, plan=plan,
        ...
        for fold in result["folds"]:
            ...
            # Без плана нет lineage-записи плана, но importance по-прежнему
            # привязан к ТОЧНОЙ матрице адаптера (plan_id = None) --
            # важность никогда не существует вне привязки к своей матрице.
            importance = fold["feature_importance"]
            assert importance is not None
...
class TestRandomForestTuning:
    ...
    def test_tuning_runs_on_same_folds_and_picks_best(self):
        ...
        response = execute_tuning_plan(
            model_id="random_forest", model_name="Random Forest",
            family_id="tree_ml",
            param_space={"n_estimators": [50, 80], "n_lags": [2, 3]},
        ...
        assert response.best_params["n_estimators"] in {50, 80}
