...
"""Task 140 -- N-HiTS: адаптер на едином NeuralForecast-runtime
(neural_runtime.py Task 137), третий исполнитель нейро-семейства.

Прецедент пары lstm/nbeats (Tasks 138/139) в neural-runtime и пары
garch/egarch в volatility-движке: ЕДИНСТВЕННАЯ точка импорта
torch/neuralforecast -- apps/api/model_impls/neural_runtime.py (лениво,
fail-closed); адаптер:
- архитектурный выбор степени иерархической интерполяции
  interpolation_config ∈ {hierarchical, light} (аналог stack_config
  Task 139): каталог декларирует «Hierarchical interpolation N-BEATS.
  Быстрее и точнее на долгих горизонтах» -- hierarchical = канонический
  N-HiTS ( Challu et al. 2023; официальные дефолты 3.2.2:
  n_pool_kernel_size=[2,2,1], n_freq_downsample=[4,2,1] -- агрессивная
  интерполяция длинных горизонтов), light = минимальная иерархия
  (n_pool_kernel_size=[2,1,1], n_freq_downsample=[2,1,1] -- фактор 2
  только на самом грубом стеке, максимум разрешения коротких
  горизонтов); дефолт -- hierarchical;
- ds-ось -- ЗАДЕКЛАРИРОВАННАЯ конвенция neural-семейства,
  ...
  сетка (freq=1, подтверждено пробом scripts/task140_nhits_probe.py);
- интервалы -- официальный conformal-контур контракта Task 137
  (fit prediction_intervals + predict level), уровни из
  interval_levels_for_alpha; NHITS 3.2.2 -- point-loss модель
...
Бюджет: NHITS_MAX_STEPS -- КОНСТАНТА модуля (единый бюджетный рычаг
...
def test_default_params_are_declared_and_normalization_fills_defaults():
    normalized = validate_nhits_params(None)
    ...
    merged = validate_nhits_params(
        {"tbats_seasonal_periods": 12, "interpolation_config": "light"}
    )
    assert merged["interpolation_config"] == "light"
    assert "tbats_seasonal_periods" not in merged


def test_unknown_interpolation_config_is_rejected_fail_closed():
    with pytest.raises(ValueError, match="interpolation_config"):
        validate_nhits_params({"interpolation_config": "wavelet"})
    assert set(INTERPOLATION_OPTIONS) == {"hierarchical", "light"}


def test_numeric_params_outside_bounds_are_rejected():
    with pytest.raises(ValueError, match="hidden_size"):
        validate_nhits_params({"hidden_size": 4})
    with pytest.raises(ValueError, match="hidden_size"):
        validate_nhits_params({"hidden_size": 10_000})
    with pytest.raises(ValueError, match="mlp_layers"):
        validate_nhits_params({"mlp_layers": 0})
    with pytest.raises(ValueError, match="mlp_layers"):
        validate_nhits_params({"mlp_layers": 5})
    with pytest.raises(ValueError, match="input_size"):
        validate_nhits_params({"input_size": 2})
...
def test_fit_predict_payload_contract(fast_budget):
    payload = _nhits_fit_predict(_series(), 5)
    assert payload["adapter_id"] == NHITS_ADAPTER_ID
    ...
    assert payload["interpolation_config"] == "hierarchical"
    assert payload["deterministic"] is True
    assert payload["params"]["alpha"] in ALPHA_OPTIONS


def test_fit_predict_light_config_runs_on_the_same_runtime(fast_budget):
    payload = _nhits_fit_predict(
        _series(), 4, params={"interpolation_config": "light"},
    )
    assert payload["interpolation_config"] == "light"
    assert len(payload["forecast"]) == 4


def test_interpolation_kwargs_are_the_honest_catalog_alternative():
    """Обе конфигурации -- bounded-поверхность официального конструктора
    3.2.2 (проб scripts/task140_nhits_probe.py): hierarchical --
    официальные дефолты библиотеки (агрессивная интерполяция длинных
    горизонтов), light -- минимальная иерархия (фактор 2 только на
    самом грубом стеке).  Оба механизма N-HiTS активны в обеих:
    multi-rate pooling + hierarchical interpolation."""
...
def test_conformal_intervals_match_the_alpha_plan(fast_budget):
    ...
    payload = _nhits_fit_predict(_series(), 4, params={"alpha": alpha})
...
def test_conformal_request_and_extraction_use_width_semantics(monkeypatch, fast_budget):
    ...
    def fake_train(*, model_factory, config, levels=(), **_kwargs):
        ...
        return pd.DataFrame({
            "NHITS": [1.0] * 4,
            "NHITS-lo-2.5": [0.9] * 4,     # дефектная колонка (48.75-й процентиль)
            "NHITS-hi-97.5": [1.1] * 4,    # дефектная колонка (98.75-й процентиль)
            "NHITS-lo-95.0": [0.0] * 4,    # ширина 95: 2.5-й процентиль
            "NHITS-hi-95.0": [2.0] * 4,    # ширина 95: 97.5-й процентиль
        })

    monkeypatch.setattr(nhits_module, "train_and_forecast", fake_train)
    payload = _nhits_fit_predict(_series(), 4)
...
def test_same_seed_gives_bit_identical_forecast(fast_budget):
    a = _nhits_fit_predict(_series(), 4, random_state=21)
    b = _nhits_fit_predict(_series(), 4, random_state=21)
    np.testing.assert_array_equal(
...
def test_different_seed_gives_different_forecast(fast_budget):
    ...
    a = _nhits_fit_predict(_series(), 4, random_state=21)
    b = _nhits_fit_predict(_series(), 4, random_state=31337)
    assert float(np.abs(
        np.asarray(a["forecast"]) - np.asarray(b["forecast"]),
    ).max()) > 0.0
...
def test_datetime_labels_infer_frequency_axis(fast_budget):
    ...
    payload = _nhits_fit_predict(_series(), 4, timestamps=labels)
...
def test_budget_wiring_reaches_the_constructor(monkeypatch, fast_budget):
    ...
    with pytest.raises(_WiringProbeDone):
        _nhits_fit_predict(_series(), 4)
    assert captured["config"].max_steps == nhits_module.NHITS_MAX_STEPS
...
def test_fault_injected_broken_bounds_are_rejected(monkeypatch, fast_budget):
    ...
    def broken_train(*, model_factory, **_kwargs):
        ...
        return pd.DataFrame({
            "NHITS": [1.0, 1.0, 1.0, 1.0],
            "NHITS-lo-95.0": [5.0] * 4,
            "NHITS-hi-95.0": [6.0] * 4,
        })

    monkeypatch.setattr(nhits_module, "train_and_forecast", broken_train)
...
# ── 7. Legacy synthetic-demo dispatch (прецедент lstm/nbeats) ────────────

def test_legacy_backtest_returns_real_metrics(fast_budget):
    metrics = run_nhits_backtest(_series(96), 0.75, 12)
    assert isinstance(metrics, BacktestMetrics)
    assert metrics.mae is not None and metrics.mae >= 0


def test_legacy_backtest_short_series_fails_honestly(fast_budget):
    # 10 точек -> train 8 < NHITS_MIN_TRAIN: честный отказ, БЕЗ Naive-подмен.
    with pytest.raises(ValueError, match="слишком короткая"):
        run_nhits_backtest(_series(10), 0.8, 12)


def test_training_config_carries_the_adapter_budget(fast_budget):
    config = NeuralTrainingConfig(seed=1, max_steps=NHITS_MAX_STEPS)
    assert config.as_dict()["max_steps"] == NHITS_MAX_STEPS


# ── 8. Env-рычаг бюджета слабых инстансов (паттерн Task 138c/139) ────────
...
def test_env_max_steps_default_override_and_fail_closed(monkeypatch):
    ...
    from apps.api.model_impls.nhits import _resolve_max_steps
