"""Task 129 -- LightGBM в движке бэктеста и тюнинга (real registry).

E2E-контракт вертикального среза (зеркало Task 127/128):
- lightgbm зарегистрирован в MODEL_EXECUTION_REGISTRY как supervised-адаптер
...
def _backtest_plan() -> tuple[object, list[float], list[str]]:
    ...
    backtest_plan = build_backtest_plan(
        ...
        fingerprint="task129-lgb", target_column="value", seasonal_period=12,
        feature_plan=plan, feature_columns=_feature_columns(labels),
    )
    return backtest_plan, values, labels


LGB_PARAMS = {"n_estimators": 40, "n_lags": 3}
...
class TestLightgbmRegistry:
    def test_registered_as_supervised_ml_adapter(self):
        descriptor = MODEL_EXECUTION_REGISTRY.describe("lightgbm")
        assert descriptor["family_id"] == "tree_ml"
        assert descriptor["adapter_id"] == "lightgbm-native"
        ...
        assert descriptor["engine"] == "lightgbm"
        assert descriptor["required_packages"] == ["lightgbm"]
        assert descriptor["runtime_available"] is True
        assert set(descriptor["actions"]) == {"backtest", "tune", "diagnostics"}

    def test_production_actions_include_lightgbm(self):
        assert "lightgbm" in PRODUCTION_BACKTEST_MODEL_IDS
        assert "lightgbm" in PRODUCTION_TUNING_MODEL_IDS
...
class TestLightgbmBacktest:
    ...
    def test_folds_record_importance_bound_to_exact_matrix(self):
        ...
        for fold in result["folds"]:
            ...
            assert "lgb_lag_1" in names
            assert "date_month_sin" in names
            total = sum(item["importance"] for item in importance["importances"])
            # Контракт нормализации: адаптер приводит сырые gain-суммы
            # LightGBM к сумме 1.0.
    ...
    def test_run_without_feature_plan_still_works(self):
        ...
        plan = build_backtest_plan(
            _validation(len(values)), n_observations=len(values),
            fingerprint="task129-lgb-bare", target_column="value", seasonal_period=12,
        )
        result = run_backtest_plan(
            model_id="lightgbm", model_name="LightGBM",
            family_id="tree_ml", series=values, labels=labels, plan=plan,
