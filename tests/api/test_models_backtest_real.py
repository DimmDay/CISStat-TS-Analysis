...
class TestBacktestImplementationsRegistry:
    """Регрессия: _BACKTEST_IMPLEMENTATIONS должен содержать все 12 моделей:
    4 baseline + 8 production. Если кто-то случайно удалил ключ — тест падает."""

    def test_registry_has_12_implementations(self):
        ...
        assert set(impls.keys()) == expected, (
            f"Expected 12 implementations: {expected}, got: {set(impls.keys())}"
        )
