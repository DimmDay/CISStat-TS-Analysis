...
class TestModelImplsModuleDirectly:
    ...
    def test_tbats_impl_callable_with_minimal_series(self):
        """Task 125: run_tbats_backtest — legacy synthetic-demo эндпоинт
        (одиночный period, без multi-period spectral hand-off)."""
        from apps.api.model_impls import run_tbats_backtest
        from apps.api.schemas import BacktestMetrics

        series = [100 + 5 * t + 20 * math.sin(2 * math.pi * t / 12)
                  for t in range(24)]

        metrics = run_tbats_backtest(series, train_ratio=0.8, seasonal_period=12)
        assert isinstance(metrics, BacktestMetrics)
        assert metrics.mae >= 0
        assert metrics.rmse >= 0
        assert 0 <= metrics.weighted_score <= 1.0
...
class TestRealModelsHandleEdgeCases:
    ...
    def test_short_series_does_not_500(self):
        ...
        for model_id in ["ets", "ets_damped", "theta", "arima", "arima_auto", "prophet", "tbats"]:
            resp = client.post(
                "/v1/internal/models/backtest",
                json={
                    "model_id": model_id,
...
class TestBacktestImplementationsRegistry:
    """Регрессия: _BACKTEST_IMPLEMENTATIONS должен содержать все 19 моделей:
    4 baseline + 15 production. Если кто-то случайно удалил ключ -- тест падает."""

    def test_registry_has_19_implementations(self):
        ...
        expected = {
            ...
            "vecm",  # Task 133 (multivariate; fold-local ранг Йохансена)
            "garch",  # Task 135 (volatility; target -- условная дисперсия)
            "egarch",  # Task 136 (volatility; leverage/asymmetry)
        }
        # Task 138/139/140/141: lstm, nbeats, nhits и tft -- условные
        # neural-записи (опциональная группа requirements-neural.txt; см.
