# app/core/metrics.py
"""
Модуль для расчета метрик и паспортов свойств временных рядов.
"""
import numpy as np
import pandas as pd
...
def calculate_ts_passport(
    ...
    Args:
        analysis_series: Временной ряд (pd.Series с DatetimeIndex).
        df_filtered: DataFrame с числовыми колонками для расчета корреляций (опционально).
        ct_f: Словарь с типами колонок (опционально).
        target_col: Имя целевой колонки для расчета корреляций (опционально).
        error_log: Список для логирования ошибок (опционально). Если не передан, ошибки не логируются.
        
    Returns:
        dict: Словарь с метриками паспорта свойств.
    """
    ...
    try:
        ...
        try:
            ...
            error_log.append({'stage': 'metrics', 'severity': 'warning', 'error_type': 'FreqInferenceError', 'message': str(e)})
            inferred_freq = None
        ...
        if df_filtered is not None and ct_f is not None and target_col:
            try:
                ...
                error_log.append({'stage': 'metrics', 'severity': 'warning', 'error_type': 'CorrelationError', 'message': str(e)})
        ...
        try:
            ...
            error_log.append({'stage': 'metrics', 'severity': 'warning', 'error_type': 'STLError', 'message': str(e)})
        ...
        try:
            ...
            error_log.append({'stage': 'metrics', 'severity': 'warning', 'error_type': 'ACFError', 'message': str(e)})
        ...
        def hurst_exponent(series, max_lag=20):
            ...
            try:
                ...
                error_log.append({'stage': 'metrics', 'severity': 'warning', 'error_type': 'HurstError', 'message': str(e)})
                return 0.5
        ...
        try:
            ...
            error_log.append({'stage': 'metrics', 'severity': 'warning', 'error_type': 'FFTError', 'message': str(e)})
        ...
        try:
            ...
            error_log.append({'stage': 'metrics', 'severity': 'warning', 'error_type': 'PeriodogramError', 'message': str(e)})
        ...
        try:
            ...
            error_log.append({'stage': 'metrics', 'severity': 'warning', 'error_type': 'PyWTNotInstalled', 'message': 'PyWavelets not installed'})
            props['wavelet'] = {'scales': [], 'count': 0}
        except Exception as e:
            error_log.append({'stage': 'metrics', 'severity': 'warning', 'error_type': 'WaveletError', 'message': str(e)})
            props['wavelet'] = {'scales': [], 'count': 0}
