...
def _normalize_importances(raw_importances: np.ndarray, n_columns: int) -> list[float]:
    ...
    fallback, чтобы fold-артефакт всегда был привязываем.  Константный
    target -- другая деградация: CatBoost отказывается обучаться
    (CatBoostError), она fail-closed на уровне библиотеки (см. докстринг
    модуля).
    """
