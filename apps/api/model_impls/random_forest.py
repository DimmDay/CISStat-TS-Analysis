...
# Минимум usable-строк supervised-матрицы: меньше -- RF не обучается
...
def rf_feature_specs(n_lags: int) -> dict[str, FeatureSpec]:
    ...
    Имена префиксованы ``rf_`` и не могут столкнуться с колонками
    платформенного каталога генерации признаков.
    """
    if not PARAM_BOUNDS["n_lags"][0] <= int(n_lags) <= PARAM_BOUNDS["n_lags"][1]:
        raise ValueError(f"n_lags={n_lags} вне bounded диапазона {PARAM_BOUNDS['n_lags']}")
    specs: dict[str, FeatureSpec] = {}
    for lag in range(1, int(n_lags) + 1):
        specs[f"rf_lag_{lag}"] = FeatureSpec(
            name=f"rf_lag_{lag}", kind=KIND_LAG, role=ROLE_HISTORIC, lookback=lag,
        )
    specs["rf_roll_mean"] = FeatureSpec(
        name="rf_roll_mean", kind=KIND_ROLLING, role=ROLE_HISTORIC,
        lookback=int(n_lags), params={"statistic": "mean"},
    )
    specs["rf_roll_std"] = FeatureSpec(
        name="rf_roll_std", kind=KIND_ROLLING, role=ROLE_HISTORIC,
        lookback=int(n_lags), params={"statistic": "std"},
    )
    specs["rf_diff_1"] = FeatureSpec(
        name="rf_diff_1", kind=KIND_DIFFERENCE, role=ROLE_HISTORIC,
...
def _matrix_digest(payload: Mapping[str, Any]) -> str:
...
def supervised_matrix(
    ...
    *,
    ...
    if len(y) - warmup < MIN_USABLE_ROWS:
        raise ValueError(
            f"Random Forest: недостаточно истории для supervised-матрицы: "
