...
def _task_criterion(model: FamilyModel, family: Family, task: Task) -> dict[str, Any]:
    if task == "volatility":
        ...
        if family.id == "multivariate" and model.id in PRODUCTION_BACKTEST_MODEL_IDS:
            ...
            blocking = False
        # Task 135/136: production volatility-модели (GARCH, EGARCH)
        # исполняются как прогноз УСЛОВНОЙ ДИСПЕРСИИ доходностей target-
        # ряда -- другой объект прогноза, чем уровень, но честно
        # изолированный платформой: volatility-cohort не смешивается с
        # level-моделями в comparison (aligned_oof отвергает смешение
        # objective), метрики -- QLIKE (контракт Task 134), исполнение --
        # volatility-движок (обе модели -- в одном движке, прецедент
        # var/vecm).
