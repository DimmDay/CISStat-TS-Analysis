...
describe("EdaStationarityOverview", () => {
  ...
  it("переключатели представлений следуют бейдж-паттерну «Обзора» «Предобработки»", () => {
    ...
    const tablist = screen.getByRole("tablist", { name: "Представления стационарности" });
    ...
    const active = screen.getByRole("tab", { name: "Ряд и μ" });
    ...
    const inactive = screen.getByRole("tab", { name: "Скользящее σ" });
