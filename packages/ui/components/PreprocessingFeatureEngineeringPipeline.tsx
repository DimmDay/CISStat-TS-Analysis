...
export function PreprocessingFeatureEngineeringPipeline({ column, profile, onApplied }: { column: string | null; profile: FeatureGenerationProfile | null; onApplied: () => void }) {
  ...
  if (!column) return <div role="status" className="flex h-[420px] items-center justify-center rounded-lg bg-neutral-50 text-sm text-neutral-600">Выберите числовой target.</div>;
  if (!profile) return <div role="status" className="flex h-[420px] items-center justify-center rounded-lg bg-neutral-50 text-sm text-neutral-600">Сначала дождитесь профиля генерации признаков.</div>;
