...
describe("EdaDescriptiveOverview", () => {
  ...
  it("переключатели представлений следуют бейдж-паттерну «Обзора» «Предобработки»", () => {
    render(<EdaDescriptiveOverview profile={PROFILE} activeFeature="Price" loading={false} error={null} noDataset={false} />);

    const tablist = screen.getByRole("tablist", { name: "Представления описательных статистик" });
    ...
    const inactive = screen.getByRole("tab", { name: "Гистограмма" });
