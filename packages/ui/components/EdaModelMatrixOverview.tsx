...
// expandedChartId (правка C); тепловая карта и график семейств обёрнуты
// в ExpandableChartPanel (таблицы Shortlist/Details — без панели).
...
function FamilyChart({ profile }: { profile: EdaModelMatrixResponse }) {
  ...
  return <div role="img" aria-label="Сводка применимости по семействам" className="min-h-0 flex-1 p-3"><ResponsiveContainer width="100%" height="100%"><BarChart data={data} layout="vertical" margin={{ left: 20, right: 18, top: 5, bottom: 5 }}><CartesianGrid strokeDasharray="3 3" stroke="var(--chart-border)" /><XAxis type="number" allowDecimals={false} tick={{ fontSize: 9 }} /><YAxis type="category" dataKey="name" width={132} tick={{ fontSize: 9 }} /><Tooltip /><Legend wrapperStyle={{ fontSize: 10 }} /><Bar dataKey="candidates" name="Совместимы" stackId="a" fill="var(--status-success-bright)" isAnimationActive={false} /><Bar dataKey="conditional" name="С оговорками" stackId="a" fill="var(--status-warning-bright)" isAnimationActive={false} /><Bar dataKey="blocked" name="Заблокированы" stackId="a" fill="var(--status-error-bright)" isAnimationActive={false} /></BarChart></ResponsiveContainer></div>;
}
...
function EdaModelMatrixOverviewInner({ profile, loading, error, noDataset, parameters, onParametersChange }: Props) {
  ...
  return <section className={`relative flex h-[468px] min-h-0 flex-col rounded-lg border border-neutral-200 bg-white feed-scroll ${expandedChartId ? "overflow-hidden" : "overflow-y-auto"}`}>
    <div className="shrink-0 border-b border-neutral-100 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><h4 className="text-sm font-semibold text-neutral-800">Матрица применимости{profile ? ` «${profile.column}»` : ""}</h4>{profile ? <p className="mt-1 text-xs text-neutral-500">{TASK_LABELS[profile.task]} · h={profile.horizon} · каталог {profile.spec_version}</p> : null}<p className="mt-1 text-[10px] text-neutral-500">Реализации: <a className="text-brand underline" href="https://www.statsmodels.org/stable/tsa/" target="_blank" rel="noreferrer">statsmodels TSA</a> · <a className="text-brand underline" href="https://nixtlaverse.nixtla.io/statsforecast/index.html" target="_blank" rel="noreferrer">StatsForecast</a> · <a className="text-brand underline" href="https://nixtlaverse.nixtla.io/neuralforecast/docs/getting-started/introduction.html" target="_blank" rel="noreferrer">NeuralForecast</a></p></div><Controls parameters={parameters} onChange={onParametersChange} /></div>{profile?.recommendation ? <p className="mt-3 rounded bg-brand-light px-3 py-2 text-xs text-neutral-700">{profile.recommendation}</p> : null}{profile?.warnings.length ? <p className="mt-2 rounded bg-amber-50 px-3 py-2 text-xs text-amber-800">{profile.warnings.join(" ")}</p> : null}{/* Task w/n-4: переключатели представлений — бейдж-паттерн «Обзора» «Предобработки» (эталон «Генерации признаков»): tablist внутри шапки, контейнер mt-3 flex flex-wrap gap-2, бейджи rounded-full px-3 py-1. Guard сохраняет прежнюю видимость: только при готовых применимых данных. */}{!loading && !error && !noDataset && profile && profile.applicable ? <div role="tablist" aria-label="Представления матрицы моделей" className="mt-3 flex flex-wrap gap-2">{TABS.map((tab) => <button key={tab.id} role="tab" aria-selected={view === tab.id} onClick={() => setView(tab.id)} className={`rounded-full border px-3 py-1 text-xs ${view === tab.id ? "border-neutral-300 bg-neutral-200 text-neutral-800" : "border-neutral-200 bg-neutral-50 text-neutral-500 hover:bg-neutral-100"}`}>{tab.label}</button>)}</div> : null}</div>
    {loading ? <div role="status" className="flex min-h-0 flex-1 items-center justify-center text-sm text-neutral-500">Проверяем требования 24 моделей…</div> : error ? <div role="alert" className="flex min-h-0 flex-1 items-center justify-center bg-red-50 px-8 text-center text-sm text-red-700">{error}</div> : noDataset ? <div role="status" className="flex min-h-0 flex-1 items-center justify-center text-sm text-neutral-600">Загрузите датасет, чтобы построить матрицу.</div> : !profile ? <div role="status" className="flex min-h-0 flex-1 items-center justify-center text-sm text-neutral-600">Выберите числовой исследуемый признак.</div> : !profile.applicable ? <div role="status" className="flex min-h-0 flex-1 items-center justify-center bg-amber-50 px-8 text-center text-sm text-amber-800">{profile.reason ?? "Матрица неприменима."}</div> : <>{view === "matrix" ? <ExpandableChartPanel chartId="model-matrix-heatmap" title="Тепловая карта применимости"><RequirementMatrix profile={profile} /></ExpandableChartPanel> : null}{view === "families" ? <ExpandableChartPanel chartId="model-matrix-families" title="Применимость по семействам"><FamilyChart profile={profile} /></ExpandableChartPanel> : null}{view === "shortlist" ? <Shortlist profile={profile} /> : null}{view === "details" ? <DetailTable profile={profile} /> : null}</>}
  </section>;
}
