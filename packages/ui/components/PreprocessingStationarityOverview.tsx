...
// ExpandableChartPanel (уровень ИСПОЛЬЗОВАНИЯ, §7.2), таблица «Кандидаты» —
...
function SeriesView({ profile }: { profile: StationarityProfile }) {
  ...
  return <div role="img" aria-label="Ряд до и после обеспечения стационарности" className="grid min-h-0 flex-1 grid-cols-2 grid-rows-1 gap-2 p-3"><div><p className="text-center text-[10px] text-neutral-500">Исходный уровень</p><ResponsiveContainer width="100%" height="92%"><LineChart data={profile.points}><CartesianGrid stroke="var(--chart-grid)" vertical={false} /><XAxis dataKey="x" tick={TICK} tickFormatter={fmtX} minTickGap={36} /><YAxis tick={TICK} width={44} /><Tooltip labelFormatter={(value: string) => fmtX(value)} /><Line dataKey="original" name="Исходный" stroke="var(--chart-axis-muted)" dot={false} isAnimationActive={false} /></LineChart></ResponsiveContainer></div><div><p className="text-center text-[10px] text-neutral-500">После: {methodLabel(profile.selected_method)}</p><ResponsiveContainer width="100%" height="92%"><LineChart data={profile.points}><CartesianGrid stroke="var(--chart-grid)" vertical={false} /><XAxis dataKey="x" tick={TICK} tickFormatter={fmtX} minTickGap={36} /><YAxis tick={TICK} width={44} /><Tooltip labelFormatter={(value: string) => fmtX(value)} /><Line dataKey="transformed" name="После" stroke="var(--chart-brand)" dot={false} connectNulls={false} isAnimationActive={false} /></LineChart></ResponsiveContainer></div></div>;
}

function RollingView({ profile }: { profile: StationarityProfile }) {
  // Task 97.4c: grid-rows-1 — аналогично SeriesView (ряд minmax(0,1fr) от
  // контейнера, а не от контента svg).
  return <div role="img" aria-label="Скользящие среднее и отклонение" className="grid min-h-0 flex-1 grid-cols-2 grid-rows-1 gap-2 p-3"><div><p className="text-center text-[10px] text-neutral-500">Rolling mean: отклонение в σ ряда</p><ResponsiveContainer width="100%" height="92%"><LineChart data={profile.points}><CartesianGrid stroke="var(--chart-grid)" vertical={false} /><XAxis dataKey="x" tick={TICK} tickFormatter={fmtX} minTickGap={38} /><YAxis tick={TICK} width={38} /><Tooltip /><Legend wrapperStyle={{ fontSize: 9 }} /><Line dataKey="rolling_mean_z_before" name="До" stroke="var(--chart-axis-muted)" dot={false} isAnimationActive={false} /><Line dataKey="rolling_mean_z_after" name="После" stroke="var(--chart-brand)" dot={false} isAnimationActive={false} /></LineChart></ResponsiveContainer></div><div><p className="text-center text-[10px] text-neutral-500">Rolling σ / глобальная σ</p><ResponsiveContainer width="100%" height="92%"><LineChart data={profile.points}><CartesianGrid stroke="var(--chart-grid)" vertical={false} /><XAxis dataKey="x" tick={TICK} tickFormatter={fmtX} minTickGap={38} /><YAxis tick={TICK} width={38} /><Tooltip /><Legend wrapperStyle={{ fontSize: 9 }} /><Line dataKey="rolling_std_ratio_before" name="До" stroke="var(--chart-axis-muted)" dot={false} isAnimationActive={false} /><Line dataKey="rolling_std_ratio_after" name="После" stroke="var(--status-success)" dot={false} isAnimationActive={false} /></LineChart></ResponsiveContainer></div></div>;
}
...
function TestsView({ profile }: { profile: StationarityProfile }) {
  ...
  return <div role="img" aria-label="P-значения до и после преобразования" className="min-h-0 flex-1 p-3"><p className="px-2 text-[10px] text-neutral-500">ADF/PP/ZA: p&lt;α поддерживает стационарность; KPSS: p≥α. Линия — α={profile.alpha}.</p><ResponsiveContainer width="100%" height="92%"><BarChart data={data} margin={{ top: 5, right: 10, bottom: 0, left: -10 }}><CartesianGrid stroke="var(--chart-grid)" vertical={false} /><XAxis dataKey="label" tick={TICK} /><YAxis domain={[0, 1]} tick={TICK} width={38} /><Tooltip /><Legend wrapperStyle={{ fontSize: 10 }} /><ReferenceLine y={profile.alpha} stroke="var(--status-error)" strokeDasharray="4 3" /><Bar dataKey="before_p_value" name="p до" fill="var(--chart-axis-muted)" isAnimationActive={false} /><Bar dataKey="after_p_value" name="p после" fill="var(--chart-brand)" isAnimationActive={false} /></BarChart></ResponsiveContainer></div>;
}

function AcfView({ profile }: { profile: StationarityProfile }) {
  const confidence = profile.acf[0]?.confidence_after ?? 0;
  return <div role="img" aria-label="ACF до и после преобразования" className="min-h-0 flex-1 p-3"><ResponsiveContainer width="100%" height="100%"><BarChart data={profile.acf}><CartesianGrid stroke="var(--chart-grid)" vertical={false} /><XAxis dataKey="lag" tick={TICK} /><YAxis domain={[-1, 1]} tick={TICK} width={38} /><Tooltip /><Legend wrapperStyle={{ fontSize: 10 }} /><ReferenceLine y={confidence} stroke="var(--status-warning)" strokeDasharray="3 3" /><ReferenceLine y={-confidence} stroke="var(--status-warning)" strokeDasharray="3 3" /><Bar dataKey="before" name="До" fill="var(--chart-axis-muted)" isAnimationActive={false} /><Bar dataKey="after" name="После" fill="var(--chart-brand)" isAnimationActive={false} /></BarChart></ResponsiveContainer></div>;
}

function CandidatesView({ profile }: { profile: StationarityProfile }) {
  return <div className="min-h-0 flex-1 overflow-auto p-3 feed-scroll"><table aria-label="Сравнение преобразований стационарности" className="w-full min-w-[720px] text-left text-xs"><thead className="sticky top-0 bg-white text-neutral-500"><tr><th className="p-2">Метод</th><th className="p-2">Консенсус</th><th className="p-2">Потеря N</th><th className="p-2">ADF p</th><th className="p-2">KPSS p</th><th className="p-2">ACF(1)</th><th className="p-2">Var after/before</th></tr></thead><tbody>{profile.candidates.map((item) => <tr key={item.method} className="border-t border-neutral-100"><td className="p-2 font-medium">{item.label}{!item.available && <span className="block font-normal text-amber-700">{item.reason}</span>}</td><td className="p-2">{consensusLabel(item.consensus)}</td><td className="p-2">{item.available ? item.lost_observations : "—"}</td><td className="p-2 font-mono">{fmt(item.adf_p_value)}</td><td className="p-2 font-mono">{fmt(item.kpss_p_value)}</td><td className={item.over_differencing_warning ? "p-2 font-mono text-amber-700" : "p-2 font-mono"}>{fmt(item.acf_lag1)}</td><td className="p-2 font-mono">{fmt(item.variance_ratio, 3)}</td></tr>)}</tbody></table></div>;
}

export function PreprocessingStationarityOverview(props: Props) {
  return (
    <ExpandableChartsProvider>
      <PreprocessingStationarityOverviewInner {...props} />
    </ExpandableChartsProvider>
  );
}

function PreprocessingStationarityOverviewInner({ profile, loading, error, noDataset }: Props) {
  const [view, setView] = useState<View>("series");
  const { expandedChartId } = useExpandableChartState();
  if (loading) return <div role="status" className="flex h-[468px] items-center justify-center rounded-lg bg-brand-light text-sm text-neutral-500">Выполняются ADF/KPSS и сравнение преобразований…</div>;
  ...
  if (noDataset) return <div role="status" className="flex h-[468px] items-center justify-center rounded-lg bg-neutral-50 text-sm text-neutral-600">Загрузите датасет для диагностики стационарности.</div>;
  ...
  return <section className={`relative flex h-[468px] min-h-0 flex-col rounded-lg border border-neutral-200 bg-white feed-scroll ${expandedChartId ? "overflow-hidden" : "overflow-y-auto"}`}><div className="shrink-0 border-b border-neutral-100 p-4"><div className="flex flex-wrap items-start justify-between gap-2"><div><h4 className="text-sm font-semibold text-neutral-800">{profile.column}: {consensusLabel(profile.consensus_before)} → {consensusLabel(profile.consensus_after)}</h4><p className="mt-1 text-[10px] text-neutral-500">{profile.order_source === "time_column" ? `ось ${profile.order_column}` : "порядок строк"} · {profile.frequency ?? "частота не определена"} · s={profile.seasonal_period} · {methodLabel(profile.selected_method)}</p></div><p className="max-w-[430px] text-right text-[10px] text-neutral-500">Методология: <a aria-label="statsmodels ADF" className="text-brand underline" href="https://www.statsmodels.org/stable/generated/statsmodels.tsa.stattools.adfuller.html" target="_blank" rel="noreferrer">statsmodels ADF</a> · <a aria-label="statsmodels KPSS" className="text-brand underline" href="https://www.statsmodels.org/stable/generated/statsmodels.tsa.stattools.kpss.html" target="_blank" rel="noreferrer">statsmodels KPSS</a> · <a className="text-brand underline" href="https://arch.readthedocs.io/en/stable/unitroot/generated/arch.unitroot.PhillipsPerron.html" target="_blank" rel="noreferrer">arch PP</a> · <a className="text-brand underline" href="https://docs.scipy.org/doc/scipy/reference/generated/scipy.signal.detrend.html" target="_blank" rel="noreferrer">SciPy detrend</a> · <a aria-label="FPP3 differencing" className="text-brand underline" href="https://otexts.com/fpp3/stationarity.html" target="_blank" rel="noreferrer">FPP3 differencing</a></p></div><p className="mt-2 rounded bg-brand-light px-3 py-2 text-xs text-neutral-700">{profile.recommendation}</p>{profile.warnings.length > 0 && <p className="mt-2 rounded bg-amber-50 px-3 py-2 text-xs text-amber-800">{profile.warnings.join(" ")}</p>}{/* Task w/n-3: переключатели представлений — эталонный бейдж-паттерн
          ...
          Обзора (mt-3 flex flex-wrap gap-2), геометрия px-3 py-1 text-xs. */}<div role="tablist" aria-label="Графики стационарности ряда" className="mt-3 flex flex-wrap gap-2">{TABS.map((tab) => <button key={tab.id} type="button" role="tab" aria-selected={view === tab.id} onClick={() => setView(tab.id)} className={`rounded-full border px-3 py-1 text-xs ${view === tab.id ? "border-neutral-300 bg-neutral-200 text-neutral-800" : "border-neutral-200 bg-neutral-50 text-neutral-500 hover:bg-neutral-100"}`}>{tab.label}</button>)}</div></div>{view === "series" && <ExpandableChartPanel chartId="stationarity-series" title="Ряд до и после преобразования"><SeriesView profile={profile} /></ExpandableChartPanel>}{view === "rolling" && <ExpandableChartPanel chartId="stationarity-rolling" title="Rolling μ/σ"><RollingView profile={profile} /></ExpandableChartPanel>}{view === "tests" && <ExpandableChartPanel chartId="stationarity-tests" title="P-значения тестов"><TestsView profile={profile} /></ExpandableChartPanel>}{view === "acf" && <ExpandableChartPanel chartId="stationarity-acf" title="ACF до и после"><AcfView profile={profile} /></ExpandableChartPanel>}{view === "candidates" && <CandidatesView profile={profile} />}<p className="shrink-0 px-4 pb-3 text-[9px] text-neutral-400">{profile.methodology_note}</p></section>;
}
