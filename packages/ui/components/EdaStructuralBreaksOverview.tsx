...
function Regimes({ profile }: { profile: EdaStructuralBreaksResponse }) {
  return <div role="img" aria-label={`Режимы и структурные сдвиги для ${profile.column}`} className="min-h-0 flex-1 px-2 py-3"><ResponsiveContainer width="100%" height="100%"><ComposedChart data={profile.series} margin={{ top: 8, right: 18, left: 4, bottom: 8 }}><CartesianGrid strokeDasharray="3 3" stroke="var(--chart-border)"/><XAxis dataKey="index" type="number" domain={["dataMin", "dataMax"]} tick={{ fontSize: 10 }}/><YAxis tick={{ fontSize: 10 }} width={58}/><Tooltip labelFormatter={(value) => `Наблюдение ${value}`} formatter={(value: number | string, name: string) => [typeof value === "number" ? number(value) : value, name === "value" ? "Ряд" : "Кусочно-линейная оценка"]}/><Line dataKey="value" name="Ряд" stroke="var(--chart-slate)" strokeWidth={1.2} dot={false} isAnimationActive={false}/><Line dataKey="fitted" name="Кусочно-линейная оценка" stroke="var(--chart-blue)" strokeWidth={2.2} dot={false} isAnimationActive={false}/>{profile.candidates.map((item) => <ReferenceLine key={item.index} x={item.index} stroke={item.supported ? "var(--status-error)" : "var(--status-warning-mid)"} strokeDasharray={item.supported ? undefined : "4 3"}/>)}</ComposedChart></ResponsiveContainer></div>;
}
function Cusum({ profile }: { profile: EdaStructuralBreaksResponse }) {
  return <div role="img" aria-label={`CUSUM-диагностика для ${profile.column}`} className="min-h-0 flex-1 px-2 py-3"><ResponsiveContainer width="100%" height="100%"><ComposedChart data={profile.cusum_path} margin={{ top: 8, right: 18, left: 4, bottom: 8 }}><CartesianGrid strokeDasharray="3 3" stroke="var(--chart-border)"/><XAxis dataKey="index" type="number" domain={["dataMin", "dataMax"]} tick={{ fontSize: 10 }}/><YAxis tick={{ fontSize: 10 }} width={58}/><Tooltip formatter={(value: number | string, name: string) => [typeof value === "number" ? number(value) : value, name === "value" ? "Накопленные остатки" : "Критическая граница"]}/><Line dataKey="value" stroke="var(--chart-blue)" strokeWidth={2} dot={false} isAnimationActive={false}/><Line dataKey="upper" stroke="var(--status-error)" strokeDasharray="5 3" dot={false} isAnimationActive={false}/><Line dataKey="lower" stroke="var(--status-error)" strokeDasharray="5 3" dot={false} isAnimationActive={false}/></ComposedChart></ResponsiveContainer></div>;
}
function Sensitivity({ profile }: { profile: EdaStructuralBreaksResponse }) {
  return <div role="img" aria-label={`Устойчивость точек PELT для ${profile.column}`} className="min-h-0 flex-1 px-2 py-3"><ResponsiveContainer width="100%" height="100%"><ComposedChart data={profile.sensitivity} margin={{ top: 8, right: 18, left: 4, bottom: 12 }}><CartesianGrid strokeDasharray="3 3" stroke="var(--chart-border)"/><XAxis dataKey="index" type="number" domain={[0, profile.n_observations - 1]} tick={{ fontSize: 10 }} name="Положение"/><YAxis dataKey="penalty_multiplier" type="number" domain={["dataMin", "dataMax"]} tick={{ fontSize: 10 }} width={48} name="Множитель штрафа"/><Tooltip formatter={(value: number | string, name: string) => [value, name === "penalty_multiplier" ? "Множитель штрафа" : "Положение"]}/><Scatter dataKey="penalty_multiplier" fill="var(--chart-blue)" isAnimationActive={false}/></ComposedChart></ResponsiveContainer></div>;
}
