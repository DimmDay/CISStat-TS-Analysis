...
describe("EdaModelMatrixOverview", () => {
  ...
  it("переключатели представлений следуют бейдж-паттерну «Обзора» «Предобработки»", () => {
    ...
    const tablist = screen.getByRole("tablist", { name: "Представления матрицы моделей" });
    ...
    const active = screen.getByRole("tab", { name: "Матрица" });
    ...
    const inactive = screen.getByRole("tab", { name: "Семейства" });
