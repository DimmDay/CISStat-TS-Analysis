...
// ExpandableChartPanel (уровень ИСПОЛЬЗОВАНИЯ, §7.2), таблицы «Корреляции»
// и «Методы» — без панели. Пара кривых плотности до/после — один семантический
// блок сравнения, раскрывается целиком. detail_level не заказан (Этап 5
// опционален) — раскрытие чисто визуальное, с compact-данными.
...
function PreviewView({ profile }: { profile: ScalingProfile }) {
  return <div role="img" aria-label="Ряд до и после диагностического масштабирования" className="min-h-0 flex-1 p-3"><ResponsiveContainer width="100%" height="100%"><LineChart data={profile.preview_points}><CartesianGrid stroke="var(--chart-grid)" vertical={false} /><XAxis dataKey="x" tick={TICK} tickFormatter={fmtX} minTickGap={35} /><YAxis yAxisId="original" tick={TICK} width={48} /><YAxis yAxisId="scaled" orientation="right" tick={TICK} width={42} /><Tooltip /><Legend wrapperStyle={{ fontSize: 9 }} /><Line yAxisId="original" dataKey="original" name={`${profile.focus_column ?? "X"}: исходная`} stroke="var(--chart-axis-text)" dot={false} isAnimationActive={false} /><Line yAxisId="scaled" dataKey="scaled" name={profile.recommended_method} stroke="var(--chart-violet)" dot={false} isAnimationActive={false} /></LineChart></ResponsiveContainer></div>;
}


function RangesView({ profile }: { profile: ScalingProfile }) {
  return <div className="flex min-h-0 flex-1 flex-col overflow-y-auto p-3 feed-scroll"><div role="img" aria-label="Сравнение масштабов числовых признаков" className="min-h-0 flex-1"><ResponsiveContainer width="100%" height="100%"><BarChart data={profile.range_points}><CartesianGrid stroke="var(--chart-grid)" vertical={false} /><XAxis dataKey="column" tick={TICK} interval={0} angle={-12} height={45} /><YAxis tick={TICK} width={38} label={{ value: "log10 σ", angle: -90, position: "insideLeft", fontSize: 9 }} /><Tooltip /><Legend wrapperStyle={{ fontSize: 9 }} /><Bar dataKey="log_scale_before" name="до" fill="var(--chart-axis-muted)" /><Bar dataKey="log_scale_after" name="после" fill="var(--chart-violet)" /></BarChart></ResponsiveContainer></div><p className="mt-2 shrink-0 text-[10px] text-neutral-500">Логарифмическая ось показывает, сколько порядков величины разделяет стандартные отклонения. Квартильные профили рассчитаны для {profile.box_points.length / 2 || 0} колонок.</p></div>;
}
...
function DistributionView({ profile }: { profile: ScalingProfile }) {
  ...
  return <div role="img" aria-label="Распределение до и после масштабирования" className="grid min-h-0 flex-1 grid-cols-2 grid-rows-1 gap-3 p-3"><ResponsiveContainer width="100%" height="100%"><LineChart data={profile.distribution_points}><CartesianGrid stroke="var(--chart-grid)" vertical={false} /><XAxis dataKey="x_before" tick={TICK} /><YAxis tick={TICK} width={32} /><Tooltip /><Line dataKey="density_before" name="До" stroke="var(--chart-axis-text)" dot={false} isAnimationActive={false} /></LineChart></ResponsiveContainer><ResponsiveContainer width="100%" height="100%"><LineChart data={profile.distribution_points}><CartesianGrid stroke="var(--chart-grid)" vertical={false} /><XAxis dataKey="x_after" tick={TICK} /><YAxis tick={TICK} width={32} /><Tooltip /><Line dataKey="density_after" name="После" stroke="var(--chart-violet)" dot={false} isAnimationActive={false} /></LineChart></ResponsiveContainer></div>;
}
...
type ScalingProps = { profile: ScalingProfile | null; loading: boolean; error: string | null; noDataset: boolean };

export function PreprocessingScalingOverview(props: ScalingProps) {
  return (
    <ExpandableChartsProvider>
      <PreprocessingScalingOverviewInner {...props} />
    </ExpandableChartsProvider>
  );
}

function PreprocessingScalingOverviewInner({ profile, loading, error, noDataset }: ScalingProps) {
