...
function PreviewView({ profile }: { profile: ScalingProfile }) {
  return <div role="img" aria-label="Ряд до и после диагностического масштабирования" className="h-[255px] p-3"><ResponsiveContainer width="100%" height="100%"><LineChart data={profile.preview_points}><CartesianGrid stroke="#F0F0F0" vertical={false} /><XAxis dataKey="x" tick={TICK} tickFormatter={fmtX} minTickGap={35} /><YAxis yAxisId="original" tick={TICK} width={48} /><YAxis yAxisId="scaled" orientation="right" tick={TICK} width={42} /><Tooltip /><Legend wrapperStyle={{ fontSize: 9 }} /><Line yAxisId="original" dataKey="original" name={`${profile.focus_column ?? "X"}: исходная`} stroke="#737373" dot={false} isAnimationActive={false} /><Line yAxisId="scaled" dataKey="scaled" name={profile.recommended_method} stroke="#7C3AED" dot={false} isAnimationActive={false} /></LineChart></ResponsiveContainer></div>;
}

function RangesView({ profile }: { profile: ScalingProfile }) {
  return <div className="h-[255px] overflow-y-auto p-3 feed-scroll"><div role="img" aria-label="Сравнение масштабов числовых признаков" className="h-[185px]"><ResponsiveContainer width="100%" height="100%"><BarChart data={profile.range_points}><CartesianGrid stroke="#F0F0F0" vertical={false} /><XAxis dataKey="column" tick={TICK} interval={0} angle={-12} height={45} /><YAxis tick={TICK} width={38} label={{ value: "log10 σ", angle: -90, position: "insideLeft", fontSize: 9 }} /><Tooltip /><Legend wrapperStyle={{ fontSize: 9 }} /><Bar dataKey="log_scale_before" name="до" fill="#A3A3A3" /><Bar dataKey="log_scale_after" name="после" fill="#7C3AED" /></BarChart></ResponsiveContainer></div><p className="mt-2 text-[10px] text-neutral-500">Логарифмическая ось показывает, сколько порядков величины разделяет стандартные отклонения. Квартильные профили рассчитаны для {profile.box_points.length / 2 || 0} колонок.</p></div>;
}

function DistributionView({ profile }: { profile: ScalingProfile }) {
  return <div role="img" aria-label="Распределение до и после масштабирования" className="grid h-[255px] grid-cols-2 gap-3 p-3"><ResponsiveContainer width="100%" height="100%"><LineChart data={profile.distribution_points}><CartesianGrid stroke="#F0F0F0" vertical={false} /><XAxis dataKey="x_before" tick={TICK} /><YAxis tick={TICK} width={32} /><Tooltip /><Line dataKey="density_before" name="До" stroke="#737373" dot={false} isAnimationActive={false} /></LineChart></ResponsiveContainer><ResponsiveContainer width="100%" height="100%"><LineChart data={profile.distribution_points}><CartesianGrid stroke="#F0F0F0" vertical={false} /><XAxis dataKey="x_after" tick={TICK} /><YAxis tick={TICK} width={32} /><Tooltip /><Line dataKey="density_after" name="После" stroke="#7C3AED" dot={false} isAnimationActive={false} /></LineChart></ResponsiveContainer></div>;
}
...
export function PreprocessingScalingOverview({ profile, loading, error, noDataset }: { profile: ScalingProfile | null; loading: boolean; error: string | null; noDataset: boolean }) {
  ...
  if (loading) return <div role="status" className="flex h-[420px] items-center justify-center rounded-lg bg-brand-light text-sm text-neutral-500">Сравниваем масштабы признаков и методы…</div>;
  ...
  if (noDataset) return <div role="status" className="flex h-[420px] items-center justify-center rounded-lg bg-neutral-50 text-sm text-neutral-600">Загрузите датасет для настройки масштабирования.</div>;
  if (!profile) return <div role="status" className="flex h-[420px] items-center justify-center rounded-lg bg-neutral-50 text-sm text-neutral-600">Выберите числовой target для отделения X от цели.</div>;
  if (!profile.applicable) return <div role="status" className="flex h-[420px] items-center justify-center rounded-lg bg-amber-50 px-8 text-center text-sm text-amber-800">{profile.reason ?? "Масштабирование неприменимо."}</div>;
