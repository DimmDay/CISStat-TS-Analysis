...
export function EdaSeasonalityOverview({
  ...
  if (loading) return <div role="status" className="flex h-[420px] items-center justify-center rounded-lg bg-brand-light text-sm text-neutral-500">Строим спектральный и фазовый профиль…</div>;
  ...
  if (noDataset) return <div role="status" className="flex h-[420px] items-center justify-center rounded-lg bg-neutral-50 px-8 text-center text-sm text-neutral-600">Загрузите датасет, чтобы исследовать сезонность.</div>;
