...
Стратегии «медиана/мода», «среднее/мода» и линейная интерполяция статистически корректны только при пропусках, полностью случайных (MCAR) или случайных при условии наблюдаемых данных (MAR). Если же пропуск связан с ненаблюдаемым значением самой переменной (MNAR — например, датчик не пишет показание именно в момент экстремума), заполнение центральной тенденцией смещает оценки. Легаси-версия строила тепловую карту корреляции индикаторов пропуска между колонками как MAR-диагностику; текущий бэкенд уже готовит для этого сырые данные (missing_per_row_histogram), но отдельная визуализация механизма пропусков в веб-версии пока не реализована — при выборе стратегии учитывайте это ограничение и контекст предметной области.`;
