...
function FoldTimeline({ profile }: { profile: EdaValidationStrategyResponse }) {
  ...
  return <div role="img" aria-label={`Схема временных folds для ${profile.column}`} className="min-h-0 flex-1 space-y-3 overflow-y-auto p-4">
...
function Alternatives({ profile }: { profile: EdaValidationStrategyResponse }) {
  return <div className="grid min-h-0 flex-1 content-center gap-3 overflow-y-auto p-4 md:grid-cols-3">{profile.alternatives.map((item) => <article key={item.strategy} className={`rounded border p-3 ${item.strategy === profile.strategy ? "border-brand bg-brand-light" : "border-neutral-200"}`}><div className="flex items-start justify-between gap-2"><h5 className="text-xs font-semibold text-neutral-800">{item.label}</h5><span className={`rounded px-1.5 py-0.5 text-[9px] ${item.suitable ? "bg-green-50 text-green-700" : "bg-red-50 text-red-700"}`}>{item.suitable ? "доступно" : "мало данных"}</span></div><p className="mt-2 text-[11px] text-neutral-600">{item.reason}</p><p className="mt-2 text-[10px] text-neutral-500">Требуется ≥ {item.required_observations}</p></article>)}</div>;
}
