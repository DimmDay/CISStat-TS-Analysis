...
Стационарность означает устойчивость вероятностных характеристик ряда во времени. Остановка не делает вывод по одному p-значению: тесты имеют разные нулевые гипотезы, чувствительны к спецификации детерминированных компонент и на коротких выборках обладают ограниченной мощностью.
...
2. KPSS проверяет обратную H₀: стационарность. Для KPSS p ≥ α означает, что оснований отвергнуть стационарность нет. Его табличные p-значения ограничены диапазоном [0,01; 0,10], поэтому серверная часть сообщает о достижении границы вместо создания ложной точности.
...
4. Согласованный вывод: «стационарен» — ADF(c) отвергает единичный корень и KPSS(c) не отвергает стационарность вокруг уровня; «стационарен вокруг тренда» — согласованы ADF(ct) и KPSS(ct); «нестационарен» — обе спецификации ADF и KPSS согласованы с единичным корнем; остальные комбинации помечаются как неопределённые.
...
6. Zivot–Andrews проверяет единичный корень при одном эндогенно найденном структурном разрыве. В ответе используется рассчитанный индекс точки разрыва и p-значение, а не приблизительный вручную заданный порог.
7. Скользящие среднее и стандартное отклонение — визуальная диагностика локальной стабильности, а не формальный тест. Выбранное окно измеряется в наблюдениях; систематический дрейф этих кривых помогает объяснить статистический вывод.
...
2. Серверная часть переиспользует общий содержательный детектор временной оси. Даты сортируются по возрастанию; повторяющиеся даты блокируют расчёт как вероятную панель, поскольку выбор или агрегация сущности без решения пользователя изменили бы ряд.
3. Нераспознанные и нерегулярные даты блокируют тесты единичного корня: лаги должны соответствовать равноотстоящим наблюдениям. Если дата не определена, допускается текущий порядок строк с явным предупреждением и окном в наблюдениях.
4. Пропуски и бесконечные значения не удаляются молча. Удаление сжало бы временную ось; интерфейс направляет пользователя назад в «Предобработку». Минимум — 30 конечных наблюдений; константный ряд помечается как вырожденный для проверки единичного корня и не имеющий корректного p-значения.
5. Улучшенный общий расчётный контур app.eda.stationarity используется и функцией предыдущего контура run_stationarity_tests: ADF(c/ct) → KPSS(c/ct) → настоящий PP → Zivot–Andrews → согласованный вывод.
6. Один ответ API содержит шесть строк тестов, критические значения, лаги, решение при α, точку разрыва, рекомендации, скользящие среднее и стандартное отклонение. Для большого ряда график прореживается алгоритмом LTTB с сохранением формы; сами тесты всегда рассчитываются по полному ряду.
