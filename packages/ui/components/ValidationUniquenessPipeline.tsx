...
export function ValidationUniquenessPipeline({ onApplied }: { onApplied: () => void }) {
  ...
  if (busy === "load") return <div className="flex h-[420px] items-center justify-center rounded-lg bg-brand-light text-sm text-neutral-500">Загрузка мастера…</div>;
  if (!profile?.applicable) return <div role="region" aria-label="Мастер исправления уникальности" className="flex h-[420px] items-center justify-center rounded-lg bg-brand-light px-8 text-center text-sm text-neutral-600">{profile?.applicability_message || "Правило уникальности неприменимо. Проверьте составной ключ в «Управлении правилами»."}</div>;
  if ((profile.duplicate_rows ?? 0) === 0) return <div role="region" aria-label="Мастер исправления уникальности" className="flex h-[420px] flex-col items-center justify-center rounded-lg border border-green-100 bg-green-50 px-8 text-center"><p className="font-medium text-green-700">Дубликаты не найдены</p><p className="mt-2 text-sm text-neutral-600">Исправление не требуется. Активный ключ: {uniquenessKeyLabel(profile)}.</p></div>;

  const available = (Object.keys(STRATEGIES) as Strategy[]).filter((item) => profile.supported_actions.includes(item));
  return (
    <section role="region" aria-label="Мастер исправления уникальности" className="grid h-[420px] grid-cols-2 gap-3 overflow-y-auto rounded-lg border border-neutral-200 bg-white p-3 feed-scroll">
