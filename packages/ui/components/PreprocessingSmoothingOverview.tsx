...
export function PreprocessingSmoothingOverview(props: Props) {
  return (
    <ExpandableChartsProvider>
      <PreprocessingSmoothingOverviewInner {...props} />
    </ExpandableChartsProvider>
  );
}

function PreprocessingSmoothingOverviewInner({ profile, loading, error, noDataset }: Props) {
  ...
  return <section className={`relative flex h-[468px] min-h-0 flex-col rounded-lg border border-neutral-200 bg-white feed-scroll ${expandedChartId ? "overflow-hidden" : "overflow-y-auto"}`}><div className="shrink-0 border-b border-neutral-100 p-4"><div className="flex flex-wrap items-start justify-between gap-2"><div><h4 className="text-sm font-semibold text-neutral-800">{profile.column}: {methodName(profile.selected_method)}{parameterText ? ` · ${parameterText}` : ""}</h4><p className="mt-1 text-[10px] text-neutral-500">{profile.order_source === "time_column" ? `ось ${profile.order_column}` : "порядок строк"} · {profile.frequency ?? "частота не определена"} · {profile.n_observations} наблюдений</p></div><p className="text-[10px] text-neutral-500">Методы: <a aria-label="pandas rolling" className="text-brand underline" href="https://pandas.pydata.org/docs/reference/api/pandas.Series.rolling.html" target="_blank" rel="noreferrer">pandas rolling</a> · <a className="text-brand underline" href="https://pandas.pydata.org/docs/reference/api/pandas.DataFrame.ewm.html" target="_blank" rel="noreferrer">pandas EWM</a> · <a aria-label="SciPy periodogram" className="text-brand underline" href="https://docs.scipy.org/doc/scipy/reference/generated/scipy.signal.periodogram.html" target="_blank" rel="noreferrer">SciPy periodogram</a> · <a aria-label="SciPy Savitzky–Golay" className="text-brand underline" href="https://docs.scipy.org/doc/scipy/reference/generated/scipy.signal.savgol_filter.html" target="_blank" rel="noreferrer">SciPy Savitzky–Golay</a> · <a className="text-brand underline" href="https://www.statsmodels.org/stable/generated/statsmodels.nonparametric.smoothers_lowess.lowess" target="_blank" rel="noreferrer">statsmodels LOWESS</a></p></div><p className="mt-2 rounded bg-brand-light px-3 py-2 text-xs text-neutral-700">{profile.recommendation}</p>{profile.warnings.length > 0 && <p className="mt-2 rounded bg-amber-50 px-3 py-2 text-xs text-amber-800">{profile.warnings.join(" ")}</p>}{/* Task w/n-3: переключатели представлений — эталонный бейдж-паттерн
          ...
          Обзора (mt-3 flex flex-wrap gap-2), геометрия px-3 py-1 text-xs. */}<div role="tablist" aria-label="Графики сглаживания ряда" className="mt-3 flex flex-wrap gap-2">{TABS.map((tab) => <button key={tab.id} type="button" role="tab" aria-selected={view === tab.id} onClick={() => setView(tab.id)} className={`rounded-full border px-3 py-1 text-xs ${view === tab.id ? "border-neutral-300 bg-neutral-200 text-neutral-800" : "border-neutral-200 bg-neutral-50 text-neutral-500 hover:bg-neutral-100"}`}>{tab.label}</button>)}</div></div>{view === "series" && <ExpandableChartPanel chartId="smoothing-series" title="Исходный и сглаженный ряд"><SeriesView profile={profile} /></ExpandableChartPanel>}{view === "residual" && <ExpandableChartPanel chartId="smoothing-residual" title="Удалённая компонента и ACF"><ResidualView profile={profile} /></ExpandableChartPanel>}{view === "methods" && <ExpandableChartPanel chartId="smoothing-methods" title="Сравнение методов"><MethodsView profile={profile} /></ExpandableChartPanel>}{view === "spectrum" && <ExpandableChartPanel chartId="smoothing-spectrum" title="Спектр до и после"><SpectrumView profile={profile} /></ExpandableChartPanel>}{view === "diagnostics" && <DiagnosticsView profile={profile} />}<p className="shrink-0 px-4 pb-3 text-[9px] text-neutral-400">{profile.methodology_note}</p></section>;
}
