...
function SeriesView({ profile }: { profile: SmoothingProfile }) {
  return <div role="img" aria-label="Исходный и сглаженный ряд" className="h-[270px] p-3"><ResponsiveContainer width="100%" height="100%"><LineChart data={profile.points} margin={{ top: 8, right: 12, bottom: 0, left: -8 }}><CartesianGrid stroke="#F0F0F0" vertical={false} /><XAxis dataKey="x" tick={TICK} tickFormatter={fmtX} minTickGap={34} /><YAxis tick={TICK} width={48} /><Tooltip labelFormatter={(value: string) => fmtX(value)} /><Legend wrapperStyle={{ fontSize: 10 }} /><Line dataKey="original" name="Исходный" stroke="#A3A3A3" strokeWidth={1.2} dot={false} isAnimationActive={false} /><Line dataKey="smoothed" name="Сглаженный" stroke="#2E3192" strokeWidth={2} dot={false} isAnimationActive={false} /></LineChart></ResponsiveContainer></div>;
}

function ResidualView({ profile }: { profile: SmoothingProfile }) {
  return <div className="grid h-[270px] grid-cols-2 gap-2 p-3"><div role="img" aria-label="Удалённая компонента"><ResponsiveContainer width="100%" height="100%"><LineChart data={profile.points}><CartesianGrid stroke="#F0F0F0" vertical={false} /><XAxis dataKey="x" tick={TICK} tickFormatter={fmtX} minTickGap={40} /><YAxis tick={TICK} width={42} /><Tooltip /><Line dataKey="residual" name="y − smooth" stroke="#DC2626" dot={false} isAnimationActive={false} /></LineChart></ResponsiveContainer></div><div role="img" aria-label="ACF удалённой компоненты"><ResponsiveContainer width="100%" height="100%"><BarChart data={profile.residual_acf}><CartesianGrid stroke="#F0F0F0" vertical={false} /><XAxis dataKey="lag" tick={TICK} /><YAxis domain={[-1, 1]} tick={TICK} width={38} /><Tooltip /><Bar dataKey="value" name="ACF" fill="#2E3192" isAnimationActive={false} /></BarChart></ResponsiveContainer></div></div>;
}

function MethodsView({ profile }: { profile: SmoothingProfile }) {
  const data = profile.candidates.filter((item) => item.available);
  return <div role="img" aria-label="Сравнение методов сглаживания" className="h-[270px] p-3"><ResponsiveContainer width="100%" height="100%"><BarChart data={data} margin={{ top: 8, right: 12, bottom: 0, left: -8 }}><CartesianGrid stroke="#F0F0F0" vertical={false} /><XAxis dataKey="label" tick={TICK} /><YAxis tick={TICK} width={42} /><Tooltip /><Legend wrapperStyle={{ fontSize: 10 }} /><Bar dataKey="roughness_reduction_pct" name="Снижение roughness, %" fill="#2E3192" isAnimationActive={false} /><Bar dataKey="high_frequency_reduction_pct" name="Снижение high-freq, %" fill="#16A34A" isAnimationActive={false} /></BarChart></ResponsiveContainer></div>;
}

function SpectrumView({ profile }: { profile: SmoothingProfile }) {
  return <div role="img" aria-label="Спектр до и после сглаживания" className="h-[270px] p-3"><ResponsiveContainer width="100%" height="100%"><LineChart data={profile.spectrum} margin={{ top: 8, right: 12, bottom: 0, left: -8 }}><CartesianGrid stroke="#F0F0F0" vertical={false} /><XAxis dataKey="frequency" tick={TICK} /><YAxis tick={TICK} width={48} /><Tooltip /><Legend wrapperStyle={{ fontSize: 10 }} /><Line dataKey="before" name="До" stroke="#A3A3A3" dot={false} isAnimationActive={false} /><Line dataKey="after" name="После" stroke="#2E3192" dot={false} isAnimationActive={false} /></LineChart></ResponsiveContainer></div>;
}
...
export function PreprocessingSmoothingOverview({ profile, loading, error, noDataset }: Props) {
  ...
  if (loading) return <div role="status" className="flex h-[420px] items-center justify-center rounded-lg bg-brand-light text-sm text-neutral-500">Оценивается высокочастотная составляющая…</div>;
  ...
  if (noDataset) return <div role="status" className="flex h-[420px] items-center justify-center rounded-lg bg-neutral-50 text-sm text-neutral-600">Загрузите датасет для диагностики сглаживания.</div>;
