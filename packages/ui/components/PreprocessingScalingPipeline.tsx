...
export function PreprocessingScalingPipeline({ targetColumn, profile, onApplied }: { targetColumn: string | null; profile: ScalingProfile | null; onApplied: () => void }) {
  ...
  if (!targetColumn) return <div role="status" className="flex h-[468px] items-center justify-center rounded-lg bg-neutral-50 text-sm text-neutral-600">Выберите числовой target.</div>;
  if (!profile) return <div role="status" className="flex h-[468px] items-center justify-center rounded-lg bg-neutral-50 text-sm text-neutral-600">Сначала дождитесь профиля масштабов.</div>;
