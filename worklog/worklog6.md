...
## Task NAVBG-2 -- Вкладка «Знакомство с платформой» (/navigator): боковые поля 24px трёх бейджей первой секции (паттерн главной страницы)

Дата: 2026-09-16. Синхронизация: main@77f138f (Task FC-MON-1), дерево
чистое до правки, зависимости восстановлены npm ci. Постановка тимлида:
«Три бейджа [секции «Знакомство с платформой»], растянутые на всю ширину
страницы. Сжать бейджи по ширине по паттерну главной страницы -- по бокам
зазоры справа и слева от границы страницы 24px. Сжатие должно быть
соразмерным и в результате бейджи должны быть одинаковы как по высоте,
так и по ширине. Дизайн-система не меняется». Полный цикл AGENTS.md:
TDD RED->GREEN, полная frontend-регрессия, typecheck/build обеих оболочек,
прод-смоук собранного маршрута.

### Дизайн (эталон и точка изменения)

- ЭТАЛОН: HomeHero.tsx:38 (сетка маршрутов главной) -- «grid grid-cols-1
  sm:grid-cols-2 lg:grid-cols-3 gap-5 px-6»; прецедент Task w/n
  (2026-09-12): px-6 -- собственные боковые поля сетки 24px слева/справа
  от границ фоновой коробки страницы; карточки соразмерно ужимаются
  (504px -> 488px при 1600px). Тот же класс у Block B (HomeCapabilities),
  идентичность прижата кросс-тестом.
- ТОЧКА ИЗМЕНЕНИЯ: NavigatorHero.tsx, nav «Разделы знакомства с
  платформой» (три RouteCard первой секции): классы «grid grid-cols-1
  md:grid-cols-3 gap-5» -- px-6 ОТСУТСТВОВАЛ, бейджи сидели вплотную
  к границе фоновой коробки (0px). Добавлен px-6 + комментарий паттерна.
- Геометрия: <main> (px-6) даёт отступ страницы от окна; фоновая коробка
  (обёртка NAVBG-1, rounded-2xl) = контент-бокс <main>; сетка -- прямой
  потомок обёртки без промежуточных горизонтальных паддингов, поэтому
  px-6 сетки = ровно 24px зазора от границы фоновой коробки.
- Отличие брейкпоинтов (md:grid-cols-3 против sm/lg у эталона) --
  СУЩЕСТВУЮЩЕЕ, в паттерн зазоров (gap-5/px-6) не входит, не тронуто.

### Оценка рисков и решения

1. Равенство бейджей по ширине/высоте -- механика CSS Grid (fr-колонки
   md:grid-cols-3 => равная ширина; дефолтный row stretch => равная
   высота), не классы карточек. Сжатие соразмерное: каждый из трёх
   бейджей ужимается на 48px/3 = 16px (1600px), gap-5 не меняется.
   Гардится layout-классами сетки (прецедент кросс-теста
   HomeHero <-> HomeCapabilities).
2. NavigatorHero -- shared-компонент (standalone + embedded): правка
   применяется в ОБЕИХ оболочках через packages/ui без форков (прецедент
   Task w/n); в embedded (main p-4, без фоновой коробки) бейджи получают
   те же 24px поля внутри своего контейнера -- паттерн един.
3. Класс px-6 уже в бандле (main, HomeHero, HomeCapabilities) -- риск
   purge-потери исключён.
4. Секции 2-4 страницы (AppliedTasksNavigator, chevron-ряд,
   CollapsibleHalfBadge, TsAnalysisNavigator) -- вне мандата, не тронуты;
   заголовок/подзаголовок секции (центрированные, не бейджи) -- не тронуты.
5. Дизайн-система не меняется: единственный новый класс -- px-6,
   токен Tailwind (24px), уже используемый платформой.

### TDD (RED -> GREEN)

- RED: 3 новых кейса в NavigatorHero.test.tsx (сюита «боковые поля 24px
  первой секции (паттерн главной страницы)»): (1) nav-сетка содержит
  px-6; (2) гард: ровно три прямых <a>-детей + grid-cols-1/
  md:grid-cols-3/gap-5 (равенство механикой Grid, одна строка на md+);
  (3) кросс-тест с эталоном: nav-сетка и HomeHero-сетка [Маршруты]
  имеют одинаковые gap-5/px-6 (брейкпоинты колонок -- существующее
  различие, вне паттерна зазоров). Результат RED: ровно ДВА падежа --
  кейсы (1) и (3), оба на отсутствующем px-6 (новая семантика); гард (2)
  и 23 существующих теста проходят ДО правки.
- GREEN: правка компонента (+px-6 +комментарий) -> 26/26 в файле
  (23 базлайн + 3 новых).

### Синхронизация PlatformIntroduction.test.tsx (предсущественная, не часть семантики NAVBG-2)

- Полная регрессия выявила 2 падения в PlatformIntroduction.test.tsx на
  baseline 77f138f (доказано git stash: те же 2 падения на чистом дереве
  без правок NAVBG-2). Причина -- рассинхронизация, внесённая коммитом
  ec4ffce (Task NAVIG-1): компонент получил индиго-черту (border-brand
  вместо border-neutral-200) и удалённый page-bottom-separator, а
  тест-файл продолжал кодировать старую семантику.
- По прецеденту VALID-2/пересертификации Task 133 («тесты кодировали
  устаревшую семантику -- переписаны») 2 теста переписаны под
  фактическую закоммиченную семантику компонента: (1) «full-width indigo
  rule» (border-brand, guard-ом запрещён border-neutral-200; сравнение
  отдельными классами -- подстрока "border-brand" содержит "border-b");
  (2) «NO bottom separator» (queryByTestId null). Компонент НЕ тронут --
  синхронизирован только тест-файл.

### Верификация

- Полная frontend-регрессия: **105 сюит / 1051 passed / 0 failed**
  (базлайн 77f138f -- 105/1048 с 2 предсущественными падениями
  PlatformIntroduction; арифметика: 1048 + 3 новых NAVBG-2 = 1051;
  оба предсущественных падения устранены синхронизацией тест-файла;
  регрессий вне задачи нет).
- npm run typecheck:all -- 0 ошибок (embedded + standalone);
  npm run build:all -- Compiled successfully x2 (13/13 embedded,
  17/17 standalone static pages).
- Прод-смоук собранного маршрута: apps/standalone/.next/server/app/
  navigator.html содержит class="grid grid-cols-1 md:grid-cols-3
  gap-5 px-6" на nav-сетке -- паттерн в production-манифесте.
- Backend не затронут (0 файлов .py).

### Границы Task NAVBG-2 (что осознанно НЕ сделано)

- Секции 2-4 (/navigator) -- вне мандата постановки (только первая
  секция с тремя бейджами).
- Брейкпоинты сетки (md:grid-cols-3 против sm/lg главной) -- не
  унифицировались: постановка про зазоры, не о колонках.
- Коммит/пуш НЕ выполнялись (запрет AGENTS.md); рабочее дерево
  main@77f138f + изменения Task NAVBG-2.

Изменённые/новые файлы (ZIP: download/task_navbg2_navigator_hero_badges_inset.zip):
- ИЗМЕНЁННЫЕ: packages/ui/components/NavigatorHero.tsx (px-6 на nav-сетке
  + комментарий паттерна), packages/ui/components/NavigatorHero.test.tsx
  (+3 кейса: px-6, гард трёх детей/Grid-равенства, кросс-тест с
  HomeHero), packages/ui/components/PlatformIntroduction.test.tsx
...
## Task UPLOAD-1: Паттерн зелёной подсветки пройденных остановок перенесён на степпер «Загрузки» (фоновый долг закрыт)

Дата: 2026-09-16. Синхронизация: main@7b4ac02. Постановка тимлида: перевести
зелёную подсветку степпера «Загрузки» на паттерн платформы. Паттерн «Если
остановка степпера пройдена и имеет зелёную галочку (status === "done"),
кнопка окрашивается в светло-зелёный цвет, текст становится зелёным; при
прочих статусах окраски нет» уже применён к «Моделированию» (эталон),
EDA (EDA-2), «Предобработке» (PREPR-1) и «Валидации» (VALID-1); «Загрузка»
оставалась единственной вкладкой 3-колоночного паттерна без него.

### Точка изменения

packages/ui/components/TsAnalysisUpload.tsx, степпер 5 остановок
(Превью датасета / График / Распределение / Структура / Качество): в
тернарник className степпер-кнопки добавлена ветка done —
«bg-green-50 border-green-200 text-green-800» (байт-в-байт эталон
TsAnalysisModeling/Preprocessing/Validation) + комментарий паттерна.
Активная остановка сохраняет приоритет индиго (bg-brand text-white
border-brand) при ЛЮБОМ её статусе. Статусы берутся из существующего
stopStatus (реальные, не моки): parse_warnings/порог 70 уверенности
детекции/наличие числовых колонок/teaser качества.

### Специфика «Загрузки» (учтена в гардах)

stopStatus выдаёт ровно три статуса — done / warning / pending
(skipped/error/running на этой вкладке недостижимы), поэтому гарды
покрывают warning и pending. Кейсы покрывают всю достижимую матрицу
статус x активность.

### TDD (RED -> GREEN)

- RED (2 падежа зафиксированы, оба требуют нового поведения; 30 проходят):
  TsAnalysisUpload.test.tsx — новая сюита «зелёная подсветка пройденных
  остановок степпера (паттерн Моделирования)», 5 кейсов:
  (1) done+неактивная («График») -> bg-green-50/border-green-200/
  text-green-800, соседняя warning («Структура») -> bg-white без green;
  (2) active+done («Превью датасета» активна по умолчанию) -> приоритет
  индиго, без green; (3) warning+неактивная -> белая, контраст —
  «Распределение» (done) зелёная; (4) active+warning («Качество» после
  клика) -> индиго, без green и без white; (5) pending («Распределение»
  при датасете без числовых колонок — columns_info только datetime) ->
  белая, без green.
- Механика тестов — та же, что у проходящих тестов файла:
  mockFetchSequence + dropFiles по dropzone-input, ожидание степпера через
  waitFor; кнопки остановок — getByRole по /^Лейбл/ (доступимое имя =
  лейбл + aria-label иконки StatusIcon).
- GREEN: TsAnalysisUpload.tsx — ветка done; сюита 5/5.

### Верификация

- Frontend: 102 сюиты / 989 passed / 0 failed (базлайн 984 + 5 новых);
  typecheck:all embedded+standalone OK; build:all обеих оболочек OK
  (12/12 и 16/16 страниц).
- Backend: 2539 passed / 0 failed (точное совпадение с базлайном; .py
  прод-код не тронут).
- Коммит/пуш НЕ выполнялись (запрет AGENTS.md).

Изменённые файлы (ZIP: download/task_upload1_upload_stepper_green_pattern.zip):
- packages/ui/components/TsAnalysisUpload.tsx (ветка done + комментарий
  паттерна), packages/ui/components/TsAnalysisUpload.test.tsx (+5 кейсов,
  renderUploaded-хелпер), worklog/worklog6.md (эта запись).

---

## Task NAVSTG-1 -- Секция 3 «Ключевые этапы исследования ряда» (/navigator): бегущая строка шеврон-стрелок слева направо со статичными цифрами, поля 24px двух рядов, «Для кого»/«Для чего» из селектора в статичный бейдж

Дата: 2026-09-17. Синхронизация: main@5f649b7 (правка тимлида
AppliedTasksNavigator -- иконка Eye заголовка «Обзор» секции 2), дерево
чистое до правки, зависимости восстановлены npm ci. Постановка тимлида
(3 пункта): (1) первый ряд шеврон-стрелок -- динамическая бегущая строка
как на главной странице, движение СЛЕВА НАПРАВО; нижним слоем двигаются
шеврон-стрелки, слой выше -- цифры нумерации в кружках -- статичны;
дизайн шевронов по стандарту бегущей строки главной (рамка индиго,
прозрачный фон, скорость движения); (2) ряд заголовков/описаний этапов
-- сжать соразмерно по паттерну первой секции (24px слева/справа),
высоту текстовых блоков можно увеличить; (3) ряд «Для кого»/«Для чего»
-- сжать бейджи по ширине (24px) и перевести из формата селектора в
формат статичного бейджа. Полный цикл AGENTS.md: TDD RED->GREEN, полная
frontend-регрессия, typecheck/build обеих оболочек, прод-смоук.

### Дизайн (эталон и точки изменения)

- ЭТАЛОН marquee: HomeCapabilities.tsx Block A (M-02 + правки 6-7):
  вьюпорт overflow-hidden, трек flex w-max из двух одинаковых групп,
  анимация одним transform на весь трек (сдвиг -50% = ровно одна группа
  -- бесшовный цикл), 90s linear infinite, пауза на hover, отключение
  при prefers-reduced-motion; бейджи -- border-brand, bg-transparent.
- НАПРАВЛЕНИЕ: главная движется справа налево (keyframes marquee:
  translateX(0) -> -50%); постановка требует слева направо -- НОВЫЙ
  зеркальный keyframes marqueeReverse (translateX(-50%) -> 0) +
  animation marquee-reverse («marqueeReverse 90s linear infinite») в
  packages/ui/tailwind-preset.ts -- та же скорость/характер, обратное
  направление. Класс главной animate-marquee НЕ тронут; изоляция
  направлений прижата прод-смоуком (index.html -- animate-marquee,
  navigator.html -- только animate-marquee-reverse).
- Стрелка: SVG-полигон fill="none" stroke="#2E3192" strokeWidth="2"
  vectorEffect="non-scaling-stroke" во viewBox 280x44,
  preserveAspectRatio="none"; полигон утоплен на 3 ед. от краёв viewBox
  -- обводка не срезается границами SVG при любом растяжении.
  ОБОСНОВАНИЕ техники: прежняя стрелка -- div + clip-path + сплошной
  bg-neutral-100; требуемая пара «рамка индиго + прозрачный фон» на
  clip-path реализуемы только подменой внутреннего фона цветом страницы
  (ломает прозрачность над волновым фоном NAVBG-1) или двойным
  clip-path (внутреннему слою нужна непрозрачная заливка); CSS-border
  вдоль диагоналей полигона обрезается clip-path'ом. Инлайн-SVG --
  прецедент платформы (HomeWavesBackground/NavigatorWavesBackground).
- Геометрия строки: ширина стрелки w-[clamp(160px,16vw,280px)] (на
  1600px видно ~6 стрелок -- прежний статичный ряд был 6-колоночным),
  высота h-11 прежняя, flex БЕЗ gap -- стрелки цепляются в ленту, как
  прежний бесшовный grid-ряд. Группа = 14 стрелок (как 14 бейджей
  главной marquee): 14 x 280px = 3920px >= 3840px -- бесшовность стыка
  не зависит от ширины вьюпорта.
- Цифры: НОВЫЙ статичный слой pointer-events-none absolute inset-0
  grid grid-cols-6 НАД треком (в DOM после вьюпорта -- красится выше);
  кружки 1:1 прежние (h-7 w-7 rounded-full bg-green-50 text-green-700
  text-sm font-semibold, Task 26); aria-hidden на оверлее и каждом
  кружке. pointer-events-none -- hover-пауза трека работает сквозь
  цифры. Статичность прижата тестом: оверлей НЕ внутри анимируемого
  трека + собственные классы без animate-*.
- Текстовый ряд: та же адаптивная сетка grid-cols-2/sm:3/md:6 gap-y-2
  + px-6 (24px, паттерн первой секции -- Task NAVBG-2); стрелки из
  сетки убраны (ряд полностью статичен). Разрешение «высоту текстовых
  блоков можешь увеличить» НЕ применено как фиксированные высоты:
  блоки авторазмерные, grid-строки (auto rows + stretch) выравнивают
  высоты сами; фиксированные высоты создали бы риск обрезки на узких
  вьюпортах.
- «Для кого»/«Для чего»: CollapsibleHalfBadge (Task 21) ->
  StaticHalfBadge: БЕЗ button/aria-expanded/aria-controls/стрелок-
  шевронов, текст всегда виден; визуальная DNA сохранена 1:1
  (rounded-lg, border-brand/20, bg-brand-light/40, зелёная Check,
  типографика заголовка); сетка grid-cols-1/md:grid-cols-2 gap-3 +
  px-6.
- a11y: обёртка этапов несёт aria-label="Этапы анализа" (перенесён с
  бывшего grid-ряда, на text-grid НЕ дублируется); клон-группа и все
  стрелки aria-hidden; статичные бейджи -- semantic текст без
  интерактива.

### Оценка рисков и решения

1. Ложный guard в RED: локатор closest(".rounded-lg") от метки бейджа
   в СТАРОМ компоненте возвращал саму кнопку-триггер (класс rounded-lg
   есть и у неё), а querySelectorAll внутри кнопки не ищет в самой
   кнопке -- тест «no trigger buttons» проходил фиктивно. Локатор
   уточнён до явного data-testid корня бейджа (badge-audience /
   badge-purpose) -- прецедент уточнения ассертов (NAVBG-2).
2. Соседние тесты не задеты: PlatformIntroduction.test.tsx и
   navigator page.test.tsx старую семантику бейджей/шевронов не
   кодируют (проверено grep ДО правки); page.test «нет голых id
   градиентов» не задевается -- SVG-стрелки без градиентов и id.
3. Tailwind-preset расширен АДДИТИВНО (новые keyframes/animation) --
   обе оболочки импортируют один preset (проверено); класс главной
   animate-marquee не менялся.
4. Произвольный класс w-[clamp(160px,16vw,280px)] -- JIT Tailwind
   снимает из исходника компонента; наличие в прод-CSS/HTML
   подтверждено смоуком.
5. Reduced-motion: motion-reduce:animate-none на треке (прецедент
   главной). Класс marquee-viewport (overflow-x:auto при reduced
   motion) сознательно НЕ переиспользован: стрелки -- декорация без
   текстового содержимого, ручная прокрутка не нужна (у главной она
   для читаемости тезисов ленты).
6. NavigatorHero -- shared-компонент (standalone + embedded через
   PlatformIntroduction): правка применяется в ОБОИХ оболочках без
   форков (прецедент NAVBG-2 на этом же компоненте); секция 3 --
   часть shared-композиции, постановка не ограничивала standalone
   (в отличие от фона NAVBG-1).

### TDD (RED -> GREEN)

- RED: тест-файл NavigatorHero.test.tsx переписан. Сохранены 17
  кейсов неизменной семантики (H1, маршруты, индиго-черты, 6 цифр в
  DOM, заголовки/подписи, регрессии старых формулировок, px-6 первой
  секции + кросс-тест с HomeHero). Новые/переписанные 14 кейсов:
  marquee-вьюпорт/трек (w-max, overflow-hidden); направление --
  animate-marquee-reverse ПРИОтсутствии animate-marquee; hover-пауза +
  motion-reduce; бесшовность -- ровно 2 группы по 14 стрелок, клон
  aria-hidden; дизайн стрелки -- stroke #2E3192, fill none,
  non-scaling-stroke; статичный оверлей цифр -- вне трека, без
  animate-*, absolute/inset-0, pointer-events-none; 6 кружков прежней
  геометрии (aria-hidden на каждом), grid-cols-6; px-6 текстового
  ряда + guard «нет svg в текстовой сетке»; статичные бейджи --
  тексты всегда видимы, 0 кнопок, 0 aria-expanded/aria-controls,
  DNA-классы карточки, 0 chevron-иконок, px-6 сетки бейджей. Итог
  RED: ровно 15 падений (после устранения фиктивно-зелёного guard'а
  локатора -- см. риски), 16 проходят (гарды + сохранённая семантика).
- GREEN: правки компонента и пресета -> 31/31 в файле (базлайн файла
  26: 17 сохранены, 9 переписаны/удалены по устаревшей семантике
  селектора Task 21; +5 нетто).

### Верификация

- Полная frontend-регрессия: **105 сюит / 1069 passed / 0 failed**;
  базлайн HEAD 5f649b7 замерен точно (git stash -> прогон -> pop):
  105/1064; арифметика 1064 + 5 = 1069 сходится; регрессий вне задачи
  нет.
- npm run typecheck:all -- 0 ошибок (embedded + standalone);
  npm run build:all -- Compiled successfully x2 (13/13 embedded,
  17/17 standalone static pages).
- Прод-смоук собранного маршрута (navigator.html): animate-marquee-
  reverse x1; hover:[animation-play-state:paused]; motion-reduce:
  animate-none; 28 SVG-полигонов (2 x 14) со stroke="#2E3192" /
  fill="none"; классы px-6 обоих рядов; clamp-класс стрелок.
  index.html главной -- animate-marquee жив; в CSS-бандле --
  «marquee-reverse{animation:marqueeReverse 90s linear infinite}».
- Backend не затронут (0 файлов .py).

### Границы Task NAVSTG-1 (что осознанно НЕ сделано)

- Секции 1/2/4 страницы (/navigator) -- вне мандата (только секция 3).
- Фиксированные высоты текстовых блоков не вводились (авторазмер +
  grid-stretch; см. дизайн).
- Ручная горизонтальная прокрутка при reduced-motion для декоративной
  строки не вводилась (см. риски).
- Спецификация/документация страницы -- не менялись.
- Коммит/пуш НЕ выполнялись (запрет AGENTS.md); рабочее дерево
  main@5f649b7 + изменения Task NAVSTG-1.

Изменённые/новые файлы (ZIP: download/task_navstg1_stages_marquee.zip):
- ИЗМЕНЁННЫЕ: packages/ui/components/NavigatorHero.tsx (marquee-трек
  2 x 14 SVG-шевронов + статичный оверлей цифр + px-6 двух рядов +
  StaticHalfBadge; удалены useState/CollapsibleHalfBadge/
  ChevronDown/ChevronUp), packages/ui/components/NavigatorHero.test.tsx
  (переписан: 31 кейс), packages/ui/tailwind-preset.ts (+keyframes
  marqueeReverse, +animation marquee-reverse), worklog/worklog6.md
  (этот журнал).
- Коммит/пуш НЕ выполнялись (запрет AGENTS.md).

---

## Task NAVSTG-2 -- Точечная правка стрелок бегущей строки (секция 3 «Ключевые этапы», /navigator): ширина стрелки = ширина страницы / 6, рамка тоньше (2026-09-17)

### Постановка

1. Ширина стрелок — строго по формуле: ШИРИНА СТРАНИЦЫ / 6 стрелок =
   ШИРИНА ОДНОЙ СТРЕЛКИ (прежняя эмпирическая
   w-[clamp(160px,16vw,280px)] устраняется).
2. Рамка каждой стрелки — ТОНЬШЕ (stroke-width 2 -> 1).

### Дизайн и точки изменений

- Формула реализована КОНТЕЙНЕРНЫМИ единицами CSS. Источник истины —
  вьюпорт marquee (chevron-marquee-viewport): он full-bleed, его ширина
  = ширине страницы; вьюпорт объявлен CSS-контейнером произвольным
  свойством Tailwind [container-type:inline-size]; стрелка —
  w-[calc(100cqw/6)]: 100cqw = ширина контейнера (страницы), /6 —
  ровно её шестая часть. Следствия: на странице ВСЕГДА видно ровно
  6 стрелок (как в прежнем статичном ряду); статичный оверлей цифр
  (grid-cols-6 той же ширины) даёт одну цифру точно по центру каждой
  стрелки — выравнивание сеток «по построению», а не подбором.
- Почему не %/vw: процентная ширина внутри трека w-max не разрешается
  (циклическая зависимость ширины); 100vw включает полосу прокрутки
  (стрелки были бы шире страницы на ~scrollbar/6 каждая). Контейнерные
  единицы отсчитываются от фактической layout-ширины вьюпорта.
- Почему [container-type:inline-size], а не утилита @container: ядро
  Tailwind 3.4 НЕ содержит containerType-плагин (утилита @container
  приходит из отдельного пакета @tailwindcss/container-queries, не
  подключённого в монорепо). Новая зависимость ради одного класса —
  неоправданное расширение; произвольные свойства — ядерная
  возможность v3 (смок подтверждает генерацию правила в CSS-бандле).
- Рамка тоньше: strokeWidth 2 -> 1 на SVG-полигоне. 1px — толщина
  border у stat-бейджей marquee главной (border = 1px) — вес линии
  стрелки выравнивается со стандартом главной marquee.
  vector-effect: non-scaling-stroke сохранён: 1px стабилен при
  растяжении preserveAspectRatio="none" (стрелка = страница/6).
- Бесшовность marquee не тронута: группы по 14 стрелок; при новой
  формуле группа = 14/6 ≈ 2.33 ширины страницы — ВСЕГДА шире любого
  вьюпорта (стало даже надёжнее прежних 14 × 280px = 3920px).
  Сдвиг -50% = ровно одна группа; animate-marquee-reverse (90s linear,
  слева направо), hover-пауза, motion-reduce — без изменений.
- Точки изменений: NavigatorHero.tsx (CHEVRON_ARROW_CLASS, класс
  вьюпорта, strokeWidth, комментарии), NavigatorHero.test.tsx
  (3 новых кейса + комментарии). tailwind-preset.ts НЕ менялся.

### Оценка рисков и решения

1. Container-type на вьюпорте создаёт size-containment по inline-оси:
   высота вьюпорта (auto, 44px от стрелок) не затронута (containment
   только по inline-оси); overflow-hidden сохранён — гардится тестом.
2. Поддержка cqw браузерами (с 2023, ~96%+): при отсутствии
   контейнера/поддержки cqw-единица фолбэчится в малый размер вьюпорта
   (по спецификации контейнерных единиц) — формула деградирует
   мягко (стрелка ≈ вьюпорт/6), не ломая ленту. Прежняя ширина тоже
   была вьюпорт-зависимой (16vw) — регрессии совместимости нет.
3. Ложная генерация утилиты @container: первая сборка показала,
   что class-утилита @container НЕ генерируется ядром TW 3.4
   (плагин отсутствует) — риск пойман прод-смоуком ДО приёмки,
   механизм переключён на [container-type:inline-size] (см. дизайн).
4. Соседние компоненты: lucide-иконки страницы со stroke-width="2"
   (14x14, Eye/Check и пр.) — к бегущей строке отношения не имеют,
   сознательно не тронуты (масштаб правки — только шевроны).
5. NavigatorHero — shared-компонент (standalone + embedded): правка
   применяется в ОБОИХ оболочках (смоук navigator.html обеих — пройден).

### TDD (RED -> GREEN)

- RED: в NavigatorHero.test.tsx добавлены 3 кейса: (1) вьюпорт —
  CSS-контейнер ([container-type:inline-size] + сохранённый
  overflow-hidden); (2) каждая из 14 стрелок real-группы —
  w-[calc(100cqw/6)] и guard «нет clamp(» (устранение прежней
  эмпирической ширины); (3) каждая рамка — stroke-width="1",
  guard «не 2», vector-effect сохранён.
  Итог RED: ровно 3 падения, 31 проходит (соседняя семантика не
  задета).
- GREEN: правка компонента -> 34/34 в файле (было 31; +3 нетто).

### Верификация

- Полная frontend-регрессия: **105 сюит / 1072 passed / 0 failed**;
  базлайн NAVSTG-1 — 1069; арифметика 1069 + 3 = 1072 сходится;
  регрессий вне задачи нет.
- npm run typecheck:all — 0 ошибок (embedded + standalone);
  npm run build:all — Compiled successfully x2.
- Прод-смоук navigator.html ОБОИХ оболочек: класс вьюпорта
  «[container-type:inline-size] relative overflow-hidden»; класс
  стрелок w-[calc(100cqw/6)]; 28 SVG-полигонов (2 x 14) со
  stroke-width="1" (прежняя «2» на шевронах отсутствует; остаточные
  stroke-width="2" — только lucide-иконки страницы, вне мандата).
- Смоук CSS-бандлов: сгенерированы правила
  [container-type:inline-size]{container-type:inline-size} и
  .w-[calc(100cqw/6)]{width:calc(100cqw / 6)} — в standalone И
  embedded.
- Backend не затронут (0 файлов .py).

### Границы Task NAVSTG-2 (что осознанно НЕ сделано)

- Ряды 2/3 и остальные секции страницы — вне мандата (точечная правка
  только стрелок ряда 1).
- Размер группы (14) и скорость (90s) не менялись — бесшовность и
  стандарт движения главной сохранены.
- Новые зависимости (плагин @tailwindcss/container-queries) НЕ
  добавлялись — использовано произвольное свойство ядра.
- Коммит/пуш НЕ выполнялись (запрет AGENTS.md); рабочее дерево
  main@5f649b7 + изменения NAVSTG-1 + NAVSTG-2.

Изменённые/новые файлы (ZIP: download/task_navstg2_arrow_width_formula.zip):
- ИЗМЕНЁННЫЕ: packages/ui/components/NavigatorHero.tsx
  ([container-type:inline-size] на вьюпорте, w-[calc(100cqw/6)] у
  стрелок, strokeWidth 2 -> 1, комментарии NAVSTG-2),
  packages/ui/components/NavigatorHero.test.tsx (+3 кейса, 34),
  worklog/worklog6.md (этот журнал).
- Коммит/пуш НЕ выполнялись (запрет AGENTS.md).

---

## Task NAVDET-1 -- Секция 4 «Подробная навигация по платформе» (/navigator): сжатие 3-колоночной компоновки по ширине по паттерну главной страницы (зазоры 24px), сжатие только за счёт колонки «Описание» (2026-09-17)

### Постановка

1. Сжать секцию «Подробная навигация по платформе» (3-колоночный
   layout TsAnalysisNavigator) по ширине по паттерну главной страницы:
   боковые зазоры 24px от границ страницы (слева и справа).
2. Сжатие произвести ТОЛЬКО за счёт сжатия ширины колонки «Описание».
3. Ширину колонок «Маршрут исследования» и «Этапы модуля» НЕ ТРОГАТЬ.

### Синхронизация

- Репозиторий синхронизирован с origin/main до коммита d302117
  (git reset --hard d302117; дерево чистое). Пропущенные коммиты:
  d7fa100 (Task NAVSTG-1), 2726688 (Task NAVSTG-2) — наши прежние
  задачи применены и закоммичены пользователем; локальные
  незакоммиченные копии тех же правок устранены reset'ом.
  package.json/package-lock между 5f649b7 и d302117 не менялись —
  node_modules актуален (npm ci не требовался).
- Аномалия «все файлы modified» в рабочей копии диагностирована как
  массовое переключение filemode (100644 -> 100755, контент не менял);
  core.filemode=false — шум устранён.

### Дизайн и точки изменений

- Компонент секции 4 — TsAnalysisNavigator (общий для standalone и
  embedded, рендерится внутри PlatformIntroduction). Компоновка:
  корень flex gap-[49px]; колонки: «Маршрут исследования» (w-60
  shrink-0), «Этапы модуля» (w-80 shrink-0), «Описание»+«Обзор»
  (flex-1 min-w-0).
- Правка: класс корня flex дополнен px-6 — боковые внутренние поля
  24px с каждой стороны (тот же токен, что у рядов секции 3 и
  бейджев первой секции: Task w/n/NAVBG-2/NAVSTG-1).
- Механика распределения сжатия — БЕЗ изменения ширин: w-60/w-80 с
  shrink-0 фиксируют первые две колонки (flex-сжатие им запрещено),
  правая flex-1 min-w-0 — единственная гибкая, поглощает всё сжатие
  (48px суммарно) — ровно постановка «сжатие только за счёт
  „Описание"». Ни один размерный класс колонок не менялся.
- Не тронуто сознательно: gap-[49px] (Task 23b) — сжатие не через
  зазоры; черта и H2 секции (PlatformIntroduction) остаются w-full
  по паттерну главной (NAVBG-2/NAVSTG-1: px-6 на контенте, черта
  full-bleed); mt-8, внутренние паддинги колонок, содержимое.
- Точки изменений: TsAnalysisNavigator.tsx (класс корня + комментарии),
  TsAnalysisNavigator.test.tsx (describe NAVDET-1: 3 кейса).

### Оценка рисков и решения

1. Минимальная ширина компоновки выросла 658 -> 706px
   (240 + 320 + 2x49 + 2x24): на экранах уже ~706px появляется
   горизонтальный скролл. Компонент и ДО правки не имел адаптивных
   брейкпоинтов (min была 658px) — поведение согласовано с прежним,
   адаптивизация вне мандата точечной правки.
2. Ложное падение собственного guard-теста на RED: регэксп
   not.toMatch(/gap-\[\d+px\]/) матчил и требуемый gap-[49px].
   Ассерт переписан на точное равенство списка gap-классов
   ["gap-[49px]"] — guard стал позитивным.
3. Существующие локаторы тестов (.flex.mt-8 и .flex.gap-\[49px\].mt-8)
   устойчивы к добавлению px-6 — все прежние 40 кейсов прошли без
   правок.
4. NavigatorHero/PlatformIntroduction не тронуты: секция 3 и шапка
   страницы остаются в состоянии d302117 (NAVSTG-1/2).
5. Shared-компонент — правка применяется в ОБОИХ оболочках (смок
   navigator.html обеих — пройден).

### TDD (RED -> GREEN)

- RED: в TsAnalysisNavigator.test.tsx добавлен describe
  «Task NAVDET-1» (3 кейса): (1) px-6 на корне flex — падал (класса
  нет); (2) guard «сжатие только за счёт „Описание"»: w-60+shrink-0 /
  w-80+shrink-0 / flex-1+min-w-0 — зелёный на d302117 (контракт
  зафиксирован); (3) guard «gap не тронут»: список gap-классов корня
  ровно ["gap-[49px]"] — зелёный. Итог RED: ровно 1 падение,
  42 проходят.
- GREEN: px-6 добавлен в класс корня -> 43/43 в файле (было 40;
  +3 нетто).

### Верификация

- Полная frontend-регрессия: **1075 passed / 0 failed**; базлайн
  d302117 — 1072; арифметика 1072 + 3 = 1075 сходится; регрессий вне
  задачи нет.
- npm run typecheck:all — 0 ошибок; npm run build:all — Compiled
  successfully x2.
- Прод-смоук navigator.html обеих оболочек: класс корня компоновки
  «flex gap-[49px] mt-8 px-6» присутствует; в CSS-бандле правило
  .px-6{padding-left:1.5rem;padding-right:1.5rem} (24px с каждой
  стороны).
- Backend не затронут (0 файлов .py).

### Границы Task NAVDET-1 (что осознанно НЕ сделано)

- Ширины колонок «Маршрут исследования» (w-60) и «Этапы модуля»
  (w-80) — не тронуты (постановка).
- gap-[49px], mt-8, черта/H2 секции — не тронуты.
- Адаптивность компоновки (брейкпоинты для узких экранов) — вне
  мандата (точечная правка ширины).
- Коммит/пуш НЕ выполнялись (запрет AGENTS.md); рабочее дерево
  d302117 + изменения Task NAVDET-1.

Изменённые/новые файлы (ZIP: download/task_navdet1_description_column_inset.zip):
- ИЗМЕНЁННЫЕ: packages/ui/components/TsAnalysisNavigator.tsx
  (px-6 на корне flex-компоновки + комментарии NAVDET-1),
