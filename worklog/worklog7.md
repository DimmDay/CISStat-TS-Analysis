...
## Task ID: PREPR-4 (2026-09-18) — Аудит инвалидации профилей на всех вкладках платформы (мишень: «бэкенд гейтит честно, фронт инвалидирует только по собственному refreshKey»)

Синхронизация: main@172be75 (PREPR-3 + DKT-1..4 тимлида), рабочее дерево
чистое до работы. Постановка: провести аудит проблемы PREPR-3 («профиль
каждой остановки бэкенд честно гейтит состоянием датасета, но фронт
инвалидовал профили только по собственному refreshKey») на ВСЕХ других
вкладках платформы; применить мутационные и оракул тесты на своих
данных; результат — отчётом в worklog7.md.

### Методика аудита

Для каждой вкладки построена тройка (профили, гейтящиеся состоянием
датасета/сессии на бэкенде) × (операции, мутирующие датасет/сессию) →
(механизм инвалидации фронтенда). Мутации датасета: upload (новый
dataset_id — uuid4, upload_common.py:140), применения 10 остановок
«Предобработки» и 10 проверок «Валидации» (мутации session.dataframe
in-place, dataset_id сохраняется), POST /target-column (паспорта/сессия),
бэктест/exclude (modeling state), генерация прогноза (forecasting
state). Ключевой структурный факт: AppShellProvider живёт в layout.tsx
и НЕ ремоунтится при клиентской навигации (гидратация только при F5 и
после upload), а страницы-модули ремоунтятся — поэтому stale-состояния
возможны только (а) у панелей, постоянно смонтированных ВНУТРИ вкладки
при её собственных мутациях, и (б) у всего, что живёт в контексте
(stages/activeDataset) между переходами.

### Результаты по вкладкам

- Валидация — ЭТАЛОН: все 10 пайплайнов onApplied={runValidation} +
  RulesManagementPanel onRulesApplied={runValidation} (строка 907);
  fetchValidation бампит validationVersion (строка 519), который
  получают ВСЕ 8 self-fetch Обзор-панелей. Сквозная инвалидация полная.
- EDA — PASS: 10 профилей с datasetKey (=datasetId ?? name) в deps всех
  эффектов; мутирующих датасет операций вкладка не имеет; смена target
  перечитывает колонко-зависимые профили (activeFeature в deps).
- Моделирование — PASS: контекст/кандидаты/state перечитываются
  (activeDatasetKey, fingerprint); бэктест/exclude обновляют modeling
  state (fetchModelingState).
- Прогнозирование — PASS: refreshAll (карты + история) после генерации
  прогноза и кнопкой; артефакты чужих вкладок — через ремоунт.
- Загрузка — PASS: панели живут на uploadData/activeDataset; каждая
  загрузка создаёт НОВЫЙ dataset_id → datasetKey меняется → все
  datasetKey-gated эффекты переполучают данные.
- Навигатор/Главная — PASS: превью от датасета при ремоунте; Home —
  статичная исследовательская карта (гейтов от сессии нет).
- Предобработка — НАЙДЕН ДЕФЕКТ №1 (см. ниже).
- Задачи — НАЙДЕН ДЕФЕКТ №2 (см. ниже).

### Дефект №1 (Предобработка, нарушение контракта PREPR-3): замороженные self-fetch Обзоры

PREPR-3 перенёс инвалидацию профилей на datasetVersion, но три
self-fetch Обзор-панели (PreprocessingMissingOverview/OutliersOverview/
RegularityOverview, deps [refreshKey]) остались на собственных ключах
missingRefreshKey/outliersRefreshKey/regularityRefreshKey, которые
после замены onApplied на setDatasetVersion больше НЕ бампились ни
одним сценарием применений — ключи замерли на 0. В текущей навигации
дефект маскируется: Обзор монтируется только на активной остановке, а
мастер исправления демонтирует его, поэтому при возврате панель
ремоунтится и фетчит свежие данные. Это скрытая мина (любая
реорганизация, делающая Обзор постоянно смонтированным, воспроизводит
пользовательский баг PREPR-3) и прямое нарушение зафиксированного
контракта «применение в ЛЮБОЙ остановке = инвалидация ВСЕХ профилей».
Смена режима остановки (PUT check-modes → собственный ключ, узкая зона
гард-теста PREPR-3) — легитимный второй источник инвалидации своего
профиля, поэтому удаление ключей НЕВОЗМОЖНО; решение — СУММА живых
ключей: refreshKey={xxxRefreshKey + datasetVersion} (монотонная сумма:
применение обновляет все Обзоры, смена режима — только свой).

### Дефект №2 (Задачи, системный): хаб не пересинхронизирует stages с бэкендом

TasksHub гейтит 4 карточки и TaskArtifactRibbon по stages из
AppShellContext (deriveTaskGateState — бэкенд-контракт честно derives
из сессии), но stages гидратируются только при монтировании
провайдера (F5) и после upload. Сценарий: аналитик проходит этапы в
других вкладках → открывает /tasks (клиентская навигация, провайдер не
ремоунтится) → карточки показывают устаревший гейтинг («Начните с
этапа Загрузка» при реально существующих артефактах) до перезагрузки
страницы. Это ТОЧНО класс постановки: бэкенд-гейт честный, фронт-кэш
(stages в контексте) не узнаёт об изменениях. Комментарий «хаб ЖИВОЙ
… без дополнительного бэкенда» обещал живость, которую никто не
обеспечивал. Решение: монтирование вкладки = refreshSession() (сервер
— источник истины, контракт AppShellContext; refreshSession стабилен,
эффект без циклов). TaskArtifactRibbon отдельного вызова не требует —
монтируется только внутри хаба.

### TDD (AGENTS.md: RED -> GREEN)

- RED (подтверждён ДО правок, ровно 2 падения из 7 новых тестов):
  (1) TasksHubSessionSync.test.tsx — реальный AppShellProvider (без
  мока контекста), /v1/session/current с флагом «бэкенд уже видел done»:
  хаб, смонтированный поверх устаревшего контекста, обязан пересинхро-
  низировать stages и перегейтить карточки БЕЗ ремоунта — падал на
  «карточка „Сценарии" осталась заблокированной» (репродукция дефекта
  №2, свой мок fetch — свои данные теста); (2) исходник-гард
  «no frozen keys» — падал на отсутствии живой передачи ключей трём
  Обзорам (репродукция дефекта №1). Зелёные в RED: компонентный
  контракт refreshKey→refetch трёх Обзор-панелей и пользовательский
  оракул «Обзор показывает свежий профиль сразу после применения в
  своей остановке» (фиксируют корректное поведение, маскирующее
  дефект №1), три гарда Валидации (эталон контракта: 8 живых ключей,
  ровно один бамп, 9×onApplied + onRulesApplied).
- GREEN: TsAnalysisPreprocessing.tsx — Обзорам передана сумма
  xxxRefreshKey + datasetVersion (+ комментарий контракта);
  TasksHub.tsx — useEffect [refreshSession] (монтирование = пересинхро-
  низация); стабы refreshSession в существующих моках контекста
  (TasksHub.test.tsx, standalone /tasks/page.test.tsx). 7/7 новых +
  все старые зелёные.

### Мутационная кампания (7 мутантов, все убиты — 1 падение целевого теста на мутанта)

- M1 «снять datasetVersion из deps родительского эффекта декомпозиции»
  → убит тестом PREPR-3 «blocked decomposition stop refreshes»;
- M2a «заморозить Обзор пропусков (refreshKey={missingRefreshKey})» и
  M2b «заместить ключ константой refreshKey={0}» → убиты гард-тестом
  «no frozen keys» (not.toContain с закрывающей скобкой запрещает и
  заморозку, и замещение);
- M3 «удалить refreshKey из deps эффекта MissingOverview» → убит
  компонентным контрактом «refreshKey changes refetch»;
- M4 «убрать refreshSession-эффект из TasksHub» → убит оракулом
  синхронизации (карточки остаются заблокированными);
- M5 «убрать бамп setValidationVersion» и M6 «убрать onRulesApplied=
  {runValidation}» → убиты гард-тестами Валидации. Механика кампании:
  инъекции python-патчем поверх фиксированного дерева, прогон целевого
  теста, восстановление из бэкапа (скрипт кампании scripts/prepr4_mutation.sh
  приложен в ZIP, рабочая станция — вне репозитория); ограничение
  признаётся: интеграционно замороженный ключ Обзора неотличим от
  живого при ремоунт-навигации — поэтому гард уровня исходника.

### Верификация (полная регрессия)

- npx jest — 118 сюит / 1287 passed / 0 failed (монорепо-регрессия;
  база 172be75 зелёная, +7 тестов PREPR-4).
- npm run typecheck:all — 0 ошибок; npm run build:all — Compiled
  successfully ×2 (embedded + standalone).
- Backend не затронут (0 файлов .py) — pytest-прогон не требуется
  (прецедент frontend-only задач EDA-2/PREPR-1/VALID-2/PREPR-3).

### Границы и наблюдения вне мандата

- DatasetPassportPanel (stage="exit") — не инвалидируется сознательно
  (граница PREPR-3 сохранена).
- «Пересчитать свойства после преобразования» для 6 остановок без
  onClick (pre-existing) — не тронуто (вне мандата).
- EmbeddedHome.tsx — мёртвый код: не подключён ни одним маршрутом
  (apps/embedded/app/page.tsx использует HomeHero); потребляет stages
  из контекста — при оживлении потребует того же пересинхронизационного
  эффекта. Не тронут.
- Гидратация Валидации сбрасывается по activeDataset?.name; при
  появлении постоянно смонтированных потребителей контекста в
  validation-сценариях повторной загрузки файла с тем же именем стоит
  перейти на datasetKey (=datasetId ?? name, как в EDA). Сейчас
  недостижимо (вкладки ремоунтятся).
- Системная рекомендация (вне мандата, решает тимлид): для «живых»
  витрин поверх сессии (хаб Задач, лента, будущие dashboard-панели)
  рассмотреть refreshSession on route change / SWR-поллинг как общий
  механизм вместо per-mount эффектов.
- Коммит/пуш НЕ выполнялись (запрет AGENTS.md); работа в ZIP
  download/task_prepr4_profile_invalidation_audit.zip.

Изменённые/новые файлы:
- packages/ui/components/TsAnalysisPreprocessing.tsx (сумма живых
  ключей в 3 Обзорах + комментарий контракта)
- packages/ui/components/TasksHub.tsx (пересинхронизация stages при
  монтировании вкладки)
- packages/ui/components/TsAnalysisPreprocessing.test.tsx (+3 кейса:
  компонентный контракт, исходник-гард, пользовательский оракул)
- packages/ui/components/TasksHubSessionSync.test.tsx (НОВЫЙ: оракул
  пересинхронизации хаба на реальном провайдере)
- packages/ui/components/TsAnalysisValidation.test.tsx (+3 гарда
