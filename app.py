...
if not st.session_state.authenticated:
    ...
    with c2:
        token_input = st.text_input("Токен доступа", type="password", placeholder="Введите секретный код")
...
with tab_download:
    st.markdown("""
    <div style="border-left: 3px solid #94a3b8; padding-left: 20px; margin: 20px 0;">
        <h3 style="margin: 0 0 10px 0; color: #1e293b;">"Все модели неверны, но некоторые полезны"</h3>
        <p style="margin: 0; color: #64748B; font-style: italic; font-size: 16px;">
        — Джордж Бокс, британский статистик, внёсший вклад в такие области, как контроль качества, планирование эксперимента, анализ временных рядов и байесовский вывод.
        </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with st.expander(" Справка по методу", expanded=False):
        ...
        **⚠️ Критическая корреляция (|r| ≥ 0.85):**
    ...
    with st.expander(" Справка по методам спектрального анализа", expanded=False):
        ...
        **ℹ️ Рекомендация:** Начните с ACF для подбора ARIMA, используйте FFT/Periodogram для поиска сезонности, примените Wavelet для сложных нестационарных рядов.
        """)
    ...
    with st.expander(" Справка по свойствам временных рядов", expanded=False):
        ...
        **⚠️ Почитать**
        ...
        #### **2. Детерминированность (Determinism, R² тренда)**
        ...
        **⚠️ Влияние на моделирование:**
        ...
        #### **3. Автокорреляция (Autocorrelation, Ljung-Box Test)**
        ...
        **⚠️ Влияние на моделирование:**
        ...
        #### **4. Нормальность (Normality, Jarque-Bera Test)**
        ...
        **⚠️ Влияние на моделирование:**
        ...
        #### **5. Направление тренда (Trend Direction)**
        ...
        **⚠️ Влияние на моделирование:**
        ...
        #### **6. Корреляция признаков (Feature Correlation)**
        ...
        **⚠️ Влияние на моделирование:**
        ...
        #### **7. Сезонность (сила, STL Strength)**
        ...
        **⚠️ Влияние на моделирование:**
        ...
        #### **8. Сезонные периоды (ACF Periods)**
        ...
        **⚠️ Влияние на моделирование:**
        ...
        #### **9. Долгая память (Hurst Exponent)**
        ...
        **⚠️ Влияние на моделирование:**
        ...
        #### **11. Значимые периоды (Периодограмма)**
        ...
        **⚠️ Влияние на моделирование:**
        ...
        #### **12. Доминирующие масштабы (Wavelet)**
...
with tab_validation:
    st.markdown("""
    <div style="border-left: 3px solid #94a3b8; padding-left: 20px; margin: 20px 0;">
        <h3 style="margin: 0 0 10px 0; color: #1e293b;">"Garbage in, garbage out | мусор на входе — мусор на выходе"</h3>
        <p style="margin: 0; color: #64748B; font-style: italic; font-size: 16px;">
        — в газетной статье 1957 года о работе математиков армии США Уильям Д. Меллин объяснял, что «небрежно запрограммированные» входные данные приводят к неверным выходам.
        </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with st.expander("Как работает сохранение результатов?", expanded=False):
        ...
        **Все изменения сохраняются в `st.session_state.df`** — это основной датафрейм,
        ...
        - `df_missing_work` — для модуля пропусков
        - `df_pattern_work` — для модуля форматов (regex)
        - `df_range_work` — для модуля диапазонов
        - `df_outlier_work` — для модуля выбросов

        **Все они синхронизируются с `session_state.df`** после применения стратегии!
...
with tab_preprocessing:
    ...
    <div style="padding-left: 20px; margin: 20px 0; text-align: right;">
        <p style="margin: 0 0 10px 0; color: #1e293b; line-height: 1.6; font-size: 20px; font-weight: 400;">
