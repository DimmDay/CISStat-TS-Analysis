...
Task ID: 56 — Интерактивный раздел прикладных задач

Date: 2026-08-27

Задача
Заменить заглушку «Прикладные задачи, решаемые платформой» трёхколоночным навигатором по паттерну «Подробной навигации»: предметная область → основная задача → четыре кликабельных прикладных сценария с подробным обзором. Расширить примеры из приложенного Excel до четырёх для каждой комбинации области и основной задачи.

Состояние репозитория
- Задача выполнена поверх локальной Task 55 на `HEAD 6c7a0e6`.
- Новая синхронизация не выполнялась: прямого указания тимлида на `fetch/pull` в текущем запросе не было.
- Коммиты и push не выполнялись.

Исходные данные и проектное решение
- Из `Задачи TS Analysis.xlsx` извлечены 6 предметных областей, 5 основных задач, их определения и 30 исходных прикладных примеров с пояснениями.
- Все 30 исходных примеров сохранены первыми в соответствующих ячейках матрицы. Добавлено 90 новых сценариев; итоговый каталог содержит 120 задач (`6 × 5 × 4`).
- Навигатор повторяет информационную архитектуру существующего `TsAnalysisNavigator`, но не копирует его предметный контракт: первая колонка отвечает за область, вторая — за класс аналитической задачи, третья — за конкретный пользовательский сценарий.
- На широком экране используются три колонки; на меньшей ширине они последовательно складываются, сохраняя доступность всех элементов.

Реализация
- Создан типизированный источник данных `applied-tasks.ts`: области, основные задачи, полная матрица сценариев и безопасный селектор `getAppliedTaskExamples`.
- Создан `AppliedTasksNavigator`: 6 областей в маршрутной колонке, 5 основных задач в средней колонке и 4 интерактивных бейджа в правой.
- При смене области или основной задачи выбор сценария сбрасывается на первый релевантный пример; невозможна ситуация, когда обзор показывает задачу из предыдущей комбинации.
- В обзоре выбранного бейджа отдельно показаны «Что анализируется» и «Результат для пользователя».
- Добавлены `aria-pressed`, `aria-controls`, `aria-live`, семантические списки/группы и видимый keyboard focus.
- Заглушка в `NavigatorHero` заменена новым компонентом; публичные данные, типы и компонент экспортированы из `@cisstat/ui`.

TDD и проверка
- RED: тесты зафиксировали отсутствие источника данных, компонента и интеграции вместо заглушки.
- Матрица проверяется на размеры 6/5/4, уникальность ID, полноту описаний и сохранение всех 30 исходных названий из Excel.
- UI-тесты проверяют трёхколоночную структуру, четыре бейджа, переключение области/задачи и обновление подробного обзора.
- Focused Jest — 3/3 suites, 29/29 tests PASS.
- Полный Jest — 41/41 suites, 351/351 tests PASS, 0 snapshots.
- `npm run typecheck:all` — PASS для embedded и standalone.
- Next.js production build embedded — PASS, 13/13 статических страниц.
- Next.js production build standalone — PASS, 13/13 статических страниц.
- Для обхода ограничений runtime временно использовались локальные font/memory shims; после проверки они удалены и в изменения не входят.
- `git diff --check` — PASS.

Изменённые и новые файлы текущей задачи
- packages/ui/lib/applied-tasks.ts
- packages/ui/components/AppliedTasksNavigator.tsx
- packages/ui/components/AppliedTasksNavigator.test.tsx
- packages/ui/components/NavigatorHero.tsx
- packages/ui/components/NavigatorHero.test.tsx
- packages/ui/index.ts
...
Task ID: 57 — Точечная полировка вкладки «Знакомство с платформой»

Date: 2026-08-27

Задача
Внести четыре локальные UX-правки в `/navigator`: переименовать заголовок раздела прикладных задач, добавить серые разделители над и под ним, раскрыть по умолчанию бейджи «Для кого / Для чего» и убрать декоративную секцию «Тарифы» из первой колонки подробной навигации.

Состояние репозитория
- Работа выполнена на актуальном клоне `main`, `HEAD f42575e`.
- Новая синхронизация не выполнялась: прямого указания тимлида на `fetch/pull` в текущем запросе не было.
- Коммиты и push не выполнялись.

Проектирование и риски
- Все изменения локализованы в двух общих компонентах `@cisstat/ui`, поэтому standalone и embedded получают одинаковое поведение без дублирования.
- Разделители реализованы одним полноширинным контейнером `w-full border-y border-neutral-200`, который семантически и визуально ограничивает только заголовок; каталог из 120 задач не затронут.
- Бейджи по-прежнему независимо управляются локальным состоянием и могут сворачиваться; изменено только начальное значение обоих состояний на открытое.
- Удаление тарифов включает удаление неиспользуемого `activePlan` и импортов `PLAN_DEFINITIONS/PlanName`; маршрут, кнопка «Начать анализ» и трёхколоночный layout сохранены.

Реализация
- Заголовок H2 «Прикладные задачи, решаемые платформой» заменён на «Примеры прикладных задач».
- Заголовок помещён между двумя серыми линиями во всю ширину контейнера страницы.
- «Для кого» и «Для чего» имеют `aria-expanded=true` и видимый текст при первом рендере; повторный клик сворачивает соответствующий блок.
- Из первой колонки `TsAnalysisNavigator` полностью удалены заголовок «Тарифы», radio-кнопки и декоративное состояние выбора плана.
- Комментарии компонентов синхронизированы с новым UI-контрактом.

TDD и проверка
- RED: focused Jest — 2 suites, 11 ожидаемых падений, 24 прежних теста PASS; тесты зафиксировали старый заголовок, отсутствие линий, закрытые бейджи и наличие тарифов.
- GREEN: focused Jest — 2/2 suites, 35/35 tests PASS.
- Полный Jest — 41/41 suites, 353/353 tests PASS, 0 snapshots.
- TypeScript typecheck — PASS для embedded и standalone. Из-за известного контракта TypeScript 6 для side-effect CSS imports временно использовались локальные `.d.ts`-shim'ы; после проверки они удалены и в изменения не входят.
- Next.js production build embedded — PASS, 13/13 статических страниц.
- Next.js production build standalone — PASS, 13/13 статических страниц.
...
Task ID: 62 — Единый селектор «Исследуемый признак» в EDA и Предобработке

Date: 2026-08-28

Задача
Проверить действующий контракт автодетекта исследуемого признака, исправить выбор по умолчанию во вкладке «Разведочный EDA» и восстановить сломанный селектор во вкладке «Предобработка».

Состояние репозитория
- Работа выполнена на `HEAD 9bb1cc8` поверх незакоммиченной Task 61. Новая синхронизация не выполнялась: прямого указания тимлида на `fetch/pull` в текущем запросе не было.
- Защитный `stash@{0}` предыдущей синхронизации сохранён без изменений.
- Коммиты и push не выполнялись.

Исследование действующей логики
- Единый session-контракт уже реализован через `GET/POST /v1/session/target-column` и frontend-хук `useTargetColumn`.
- Backend возвращает все числовые колонки в `available_columns` и отдельную `suggested_column`. Рекомендация — первая числовая колонка, имя которой не похоже на временную ось (`date/дата/year/год/period/период`); datetime dtype не попадает в числовой список изначально. Если все числовые колонки похожи на временные, сохраняется безопасный fallback на первую.
- При первом открытии вкладки `useTargetColumn` автоматически POST-ит рекомендацию в `AnalysisSession`; последующий ручной выбор также сохраняется через POST. Поэтому другие вкладки получают не локальный дефолт, а тот же выбранный признак.
- «Загрузка» и «Валидация» уже использовали этот контракт. EDA после Task 61 независимо выбирала первую колонку ответа `/dataset/stats`, поэтому для `Country / Year / Price` снова получался `Year`. «Предобработка» вообще оставалась на hardcoded mock-списке `price/volume/open/...`; её label не был связан с select.

Реализация
- EDA переведена с локального `useState` на `useTargetColumn`: список опций приходит из `available_columns`, выбранное значение — из session `target_column`, ручная смена сохраняется через POST.
- Загрузка `/dataset/stats` больше не перезаписывает выбор первой числовой колонкой. Таблица по-прежнему показывает статистики всех числовых признаков, а карточки и графики синхронизируются с единым target.
- Состояния загрузки/ошибки target объединены с жизненным циклом остановки «Описательные статистики»; при ошибке синхронизации показывается явное сообщение, устаревший локальный выбор не подставляется.
- Из «Предобработки» удалён mock-массив тикеров. Селектор использует тот же `useTargetColumn`, поддерживает auto-select и session POST, корректно disabled при загрузке/отсутствии числовых колонок и получил связку `label htmlFor` / `select id`.
- Числовая временная колонка не удаляется из списка: аналитик может выбрать её явно. Исключение применяется только к автоматической рекомендации, как и раньше во вкладках «Загрузка»/«Валидация».
- Комментарий контракта `useTargetColumn` обновлён: хук теперь используется четырьмя вкладками — Загрузка, Валидация, Предобработка и EDA.

Риски и защита
- Исключён локальный fallback `stats.columns[0]`, который мог перезаписать уже сохранённый пользователем target.
- Асинхронный auto-POST проверяется отдельно от GET: интерфейс ждёт подтверждённое значение `target_column`, а не только визуально выставляет рекомендацию.
- Ручная смена проверяется на фактический POST `{ column: ... }`; после перехода на другую вкладку выбор восстанавливается из сессии.
- Dataset-wide остановки «Пропуски» и «Выбросы» не стали искусственно фильтроваться target-колонкой: общий селектор сохраняет платформенный контекст, а их собственные мастера по-прежнему позволяют выбирать несколько колонок.

TDD и проверка
- RED: EDA выбирала `Year` вместо backend-рекомендации `Price`; «Предобработка» не находилась по доступному имени label и показывала mock-опции вместо реальных колонок.
- Focused Jest — 2/2 suites, 37/37 tests PASS.
- Тест EDA использует порядок `Year, Price, Volume`, проверяет default `Price`, обновление метрик после выбора `Volume` и session POST.
- Тест «Предобработки» проверяет реальные `Year/Price/Volume`, отсутствие `adj_close`, default `Price`, доступность label и session POST при ручной смене.
- Полный Jest — 44/44 suites, 377/377 tests PASS, 0 snapshots.
- TypeScript typecheck — PASS для embedded и standalone; временные CSS-shim'ы удалены.
- Next.js production build embedded — PASS, 13/13 статических страниц.
- Next.js production build standalone — PASS, 13/13 статических страниц.
- Временный memory shim после сборок удалён и в изменения не входит.
- `git diff --check` — PASS.
...
Task ID: 73 — Остановка EDA «Распределение»

Date: 2026-08-30

Задача
Спроектировать и реализовать следующую остановку вкладки «Разведочный EDA» по паттернам «IH-анализ» и «Верификация стационарности»: исследовать существующий backend, проверить и при необходимости скорректировать методологию, переиспользовать пригодные расчёты и активно использовать переключаемые визуализации в «Обзоре».

Синхронизация
- По прямому указанию выполнен безопасный fast-forward `main`: `290d2aa` → `cc2dee1b763c705a608afac11522ec1b0e9d4b3f`.
- Перед синхронизацией незакоммиченные материалы и архивы сохранены вместе с untracked-файлами; после fast-forward восстановлены только отсутствующие в upstream файлы. Совпадающие редакции Tasks 70/71 повторно поверх upstream не накладывались.
- Реализация выполнена на точном запрошенном commit `cc2dee1`; commit и push не выполнялись.

Исследование backend и корректировка методологии
- Найден `app/eda/distributions.py::detect_distribution_type`: он подбирает несколько распределений обычным K–S после оценки их параметров по той же выборке и классифицирует результат по жёстким порогам статистики. Такой K–S не имеет стандартного распределения при оценённых параметрах, поэтому существующая функция сохранена для обратной совместимости, но не используется как формальный вывод новой остановки.
- Переиспользованы существующие полноразмерные построители `apps/api/chart_data.py::build_histogram` и `build_kde`; к ним добавлены оценённая нормальная плотность и ожидаемые частоты по точной разности CDF на границах интервала.
- Методология сверена с официальной документацией SciPy, statsmodels и NIST: Shapiro–Wilk ограничен N ≤ 5000 для надёжного p-значения; асимптотический Jarque–Bera используется с N ≥ 2000, а ниже калибруется Монте-Карло; K–S выполняется в варианте Лиллиефорса для параметров, оценённых по выборке; Q–Q график рассматривается как диагностика формы, а не самостоятельное доказательство.
- К трём доступным p-значениям применяется последовательная поправка Холма. Формальный вывод дополнен величинами асимметрии/эксцесса, чтобы статистическая значимость на большой выборке не подменяла практическую величину отклонения.
- Для автокоррелированного временного ряда p-значения явно помечены диагностическими. Интерфейс разъясняет, что нормальность для модельных доверительных интервалов проверяется на остатках после обучения, а не доказывается распределением исходного ряда.
- Пропуски/∞, N < 8 и константный ряд не очищаются молча; низкокардинальный целочисленный признак визуализируется, но тесты непрерывной нормальности помечаются неприменимыми.

Backend
- `app/eda/distributions.py` расширен новым совместимым контуром `analyze_distribution`; прежняя публичная функция и её characterization-контракт сохранены.
- Добавлен `apps/api/eda_distribution.py`: один профиль объединяет статистики формы, три теста, гистограмму, KDE, Q–Q и F(x).
- Добавлены строгие response-схемы и read-only endpoint `GET /v1/session/dataset/eda-distribution?column=...&alpha=...&bins=...` с проверками сессии, колонки, числового типа и диапазонов параметров.

Frontend
- Добавлен `EdaDistributionOverview` с пятью представлениями одного API-ответа: «Гистограмма», «Плотность», «Q–Q», «F(x)», «Тесты».
- Гистограмма сопоставляет наблюдаемые и ожидаемые при нормальной форме частоты; KDE — эмпирическую и нормальную плотности; Q–Q — квантили и опорную прямую; F(x) — эмпирическую и нормальную функции; таблица показывает исходные и скорректированные по Холму p-значения, калибровку и решение.
- Встроены параметры α и числа интервалов, автоматический пересчёт при смене общего «Исследуемого признака», ручное обновление, состояния loading/error/skipped/not-applicable и шесть итоговых метрик.
- В «Панель управления» добавлены специализированные русские пояснения методологии и полного пайплайна. Английские слова оставлены только в названиях методов, алгоритмов, обозначениях и устойчивом отраслевом сленге.

TDD, тесты и сборка
- RED: Python-тесты сначала падали на отсутствующих `analyze_distribution` и `apps.api.eda_distribution`; frontend-тесты — на отсутствующем компоненте и пяти представлениях.
- Целевые Python core/adapter/API и прежний compatibility-suite: 22/22 PASS.
- Целевые Jest: 2/2 suites, 26/26 tests PASS.
- Полный frontend Jest по исходникам, без архивных копий `download/`: 56/56 suites, 493/493 tests PASS.
- TypeScript-проверка UI: PASS с `--ignoreDeprecations 6.0`; без этого флага установленный TypeScript 6 останавливается на существующих deprecated-настройках `target=ES5` и `baseUrl` в `packages/ui/tsconfig.json`, не связанных с задачей.
- Production build embedded и standalone: PASS, по 13/13 статических страниц. Временные Google Fonts/Node memory shims после проверки удалены, оба `layout.tsx` побайтно возвращены к upstream.
- Полный API-suite: 381 PASS, 20 FAIL, 1 SKIP; новый distribution API-suite 3/3 PASS. Все 20 падений относятся к существующим независимым контурам single-column CSV parsing, modeling diagnostics и ARIMA tuning.
- Полный unit-suite upstream не собирается из-за `IndentationError` в неизменённом `tests/unit/test_file_loader.py:87`. При исключении этого файла: 483 PASS, 3 FAIL, 3 ERROR; оставшиеся проблемы относятся к IH sentinel, regularity/validation и отсутствующему snapshot fixture, файлы задачей не менялись.
- Python `py_compile` и `git diff --check`: PASS.

Изменённые и новые файлы текущей задачи
- app/eda/distributions.py
- apps/api/eda_distribution.py (новый)
- apps/api/routers/session.py
- apps/api/schemas.py
- packages/ui/components/EdaDistributionOverview.tsx (новый)
- packages/ui/components/EdaDistributionOverview.test.tsx (новый)
- packages/ui/components/TsAnalysisEDA.tsx
- packages/ui/components/TsAnalysisEDA.test.tsx
- packages/ui/index.ts
- tests/unit/test_distribution_analysis.py (новый)
- tests/unit/test_eda_distribution_adapter.py (новый)
- tests/api/test_dataset_eda_distribution.py (новый)

---

Task ID: 74 — Остановка EDA «Структурные сдвиги»

Date: 2026-08-30

Задача
Синхронизировать `main` с commit `5686e4b264e5004a285b216aeb431d3346cfcb73`, исследовать существующий backend и реализовать остановку «Структурные сдвиги» по паттернам «Распределение» и «Верификация стационарности», с методологической проверкой и несколькими визуализациями в «Обзоре».

Синхронизация
- Перед обновлением все локальные tracked/untracked материалы безопасно сохранены. Выполнен fast-forward `cc2dee1` → точный запрошенный `5686e4b264e5004a285b216aeb431d3346cfcb73`.
- Upstream уже содержал код Task 73. Сравнение подтвердило совпадение с локальным пакетом; поверх него восстановлены только отсутствовавшие `worklog.md`, архивы `download/` и независимый upstream-тест IH.
- Реализация Task 74 выполнена без commit и push.

Исследование backend и оценка методологии
- Производственной остановки для нескольких структурных сдвигов не было. Найденный Zivot–Andrews в `app/eda/stationarity.py` решает другую задачу: проверяет единичный корень с одним эндогенным разрывом и не заменяет сегментацию нескольких режимов.
- Реализован комплементарный контур: CUSUM по остаткам общей линейной модели проверяет глобальную стабильность параметров; PELT с линейной функцией потерь локализует несколько изменений уровня/наклона; локальная Chow-диагностика описывает выигрыш разбиения соседних сегментов.
- Методологически важное ограничение сделано явным в API и UI: Chow рассчитывается после выбора точек PELT на тех же данных, поэтому его p-значения являются послевыборочными и не представляются независимым подтверждением. Для нескольких кандидатов применяется поправка Холма.
- Поддержанным считается только кандидат, для которого согласованы глобальный CUSUM, Chow после Холма и устойчивость точки не менее чем в 60% сетки штрафов PELT. В отдельной вкладке показывается чувствительность к множителям 0,75/1/2/3/5.
- Базовый штраф масштабирован оценкой дисперсии остатков и `log(N)`, добавлены минимальная длина сегмента, `jump` для больших рядов и защитный предел 10 точек с автоматическим повышением штрафа.
- Анализ намеренно ограничен изменениями уровня и линейного наклона. Изменения только дисперсии направляются в отдельную диагностику волатильности; автокорреляция/сезонность явно отмечены как факторы, требующие повторной проверки на остатках модели.
- Общий детектор даты переиспользован по паттерну стационарности: даты сортируются; нераспознанная/нерегулярная сетка и повторные даты блокируют расчёт; вероятная панель не агрегируется без решения пользователя. При отсутствии уверенной даты допускается порядок строк с предупреждением.
- Пропуски/∞, N < 60, константный ряд и невозможная минимальная длина не исправляются молча, а возвращают объяснимое состояние `not_applicable`.

Backend
- Добавлен `app/eda/structural_breaks.py`: CUSUM, PELT (`ruptures`, model=`linear`), локальная Chow-диагностика, Holm, кусочно-линейные сегменты, устойчивость штрафа и русские рекомендации/ограничения.
- Добавлен `apps/api/eda_structural_breaks.py`: временная ось, строгая применимость, календарные подписи кандидатов/сегментов, LTTB-прореживание только графических рядов.
- Добавлены типизированные Pydantic-схемы и read-only endpoint `GET /v1/session/dataset/eda-structural-breaks?column=...&alpha=...&min_segment=...&penalty_multiplier=...` с проверками сессии, колонки, числового типа и диапазонов.
- В корневой и API requirements добавлен `ruptures>=1.1.9`.

Frontend
- Добавлен `EdaStructuralBreaksOverview` с пятью представлениями одного ответа: «Режимы» (ряд, кусочно-линейная оценка и границы), «CUSUM» (накопленные остатки и критические границы), «Чувствительность» (точки PELT по штрафам), «Сегменты» и «Кандидаты».
- Остановка использует единый платформенный селектор «Исследуемый признак», не создаёт локальный дубликат. Параметры α, минимальной длины и штрафа автоматически обновляют профиль; добавлена ручная кнопка «Пересчитать сдвиги».
- Добавлены специализированные состояния loading/error/skipped/not-applicable, шесть итоговых метрик, статус в «Панели управления» и подробные русские тексты «Метрики и алгоритм»/«Полный пайплайн».

TDD и проверка
- RED подтверждён до реализации: Python падал на отсутствующем `app.eda.structural_breaks`, Jest — на отсутствующем `EdaStructuralBreaksOverview` и placeholder остановки.
- Core/adapter: 6/6 PASS. Проверены сдвиг уровня, сдвиг наклона, стабильный тренд без ложного режима, монотонность сложности по штрафу, пропуски/короткий/константный ряд, сортировка даты, отказ для нерегулярной сетки и панели.
- API: 2/2 PASS, включая полный профиль и валидацию сессии/колонки/параметров.
- Frontend: 2/2 suites, 28/28 tests PASS; проверены пять вкладок, общий target, параметры, тексты методологии, endpoint и ручной пересчёт.
- Полный Jest по исходникам с исключением архивных копий `download/`: 57/57 suites, 498/498 tests PASS.
- `npm run typecheck:all`: PASS для embedded и standalone.
- Production build standalone: PASS, 13/13 статических страниц. Для изолированной песочницы временно отключались Google Fonts и сбойный системный счётчик памяти Node; после сборки оба изменения полностью удалены, `layout.tsx` совпадает с upstream.
- `python compileall` и `git diff --check`: PASS.

Изменённые и новые файлы текущей задачи
- app/eda/structural_breaks.py (новый)
- apps/api/eda_structural_breaks.py (новый)
- apps/api/requirements.txt
- apps/api/routers/session.py
- apps/api/schemas.py
- packages/ui/components/EdaStructuralBreaksOverview.tsx (новый)
- packages/ui/components/EdaStructuralBreaksOverview.test.tsx (новый)
- packages/ui/components/TsAnalysisEDA.tsx
- packages/ui/components/TsAnalysisEDA.test.tsx
- packages/ui/index.ts
- requirements.txt
